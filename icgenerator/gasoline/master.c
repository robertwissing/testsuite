...
void msrInitialize(MSR *pmsr,MDL mdl,int argc,char **argv)
{
	...
	*pmsr = msr;
	...
	msr->param.dMHDAlphaB = 1.0;
	prmAddParam(msr->prm,"dMHDAlphaB",2,&msr->param.dMHDAlphaB,
				sizeof(double),"alphaB",
				"<Fixed AlphaB constant in resistivity > = 1");
#endif
#ifdef MHDCLEANING
	msr->param.dMHDCleanFac = 1.0;
	prmAddParam(msr->prm,"dMHDCleanFac",2,&msr->param.dMHDCleanFac,
				sizeof(double),"fch",
				"<Cleaning factor in cleaning> = 1");
	msr->param.dMHDPsidecFac = 1.0;
	prmAddParam(msr->prm,"dMHDPsidecFac",2,&msr->param.dMHDPsidecFac,
				sizeof(double),"psidecfac",
				"<PsidecFac constant in cleaning> = 1");
#endif
#endif
#ifdef ICGEN
        msr->param.dICFmax = 10000000000000.0;
	prmAddParam(msr->prm,"dICFmax",2,&msr->param.dICFmax,
                    sizeof(double),"ICFmax",
                    "< Used to compare new density difference with old density difference > ");
        msr->param.dICR0 = 1.0;
	...
	msr->param.dICdensprofile = 1;
        prmAddParam(msr->prm,"dICdensprofile",0,&msr->param.dICdensprofile,
                    sizeof(int),"ICdensprofile",
                    "<Density profile -> 1: uniform, 2: density discontinous, 3: density discontinous smooth, note: for more advanced symmetric density profiles, could perhaps add input table(like in case of planets)");
	msr->param.dICdensouter = 1.0;
	prmAddParam(msr->prm,"dICdensouter",2,&msr->param.dICdensouter,
                    sizeof(double),"ICdensouter",
                    "<Density in uniform/outer medium = 1");
        msr->param.dICdensinner = 1.0;
        prmAddParam(msr->prm,"dICdensinner",2,&msr->param.dICdensinner,
                    sizeof(double),"ICdensinner",
                    "<Density in inner medium = 1");
	msr->param.dICdensdir = 2;
        prmAddParam(msr->prm,"dICdensdir",0,&msr->param.dICdensdir,
                    sizeof(int),"ICdensdir",
                    "<Direction of density gradient(1: x, 2: y, 3: z, 4: r, 5: rcyl(x,y)");
	msr->param.dICdensR = 0.5;
        prmAddParam(msr->prm,"dICdensR",2,&msr->param.dICdensR,
                    sizeof(double),"ICdensR",
                    "<Range of inner(usually high) density region = 0.5");
	msr->param.dICdensRsmooth = 0.05;
        prmAddParam(msr->prm,"dICdensRsmooth",2,&msr->param.dICdensRsmooth,
                    sizeof(double),"ICdensRsmooth",
                    "<Smoothed region between densities = 0.05");
	/*	msr->param.dICTargetRelax = 1e-6;
        prmAddParam(msr->prm,"dICTargetRelax",1,&msr->param.dICTargetRelax,
                    sizeof(double),"ICTargetRelax",
                    "<Noise target for relaxation");*/
#endif
        msr->param.dnloc = 1.0;
        prmAddParam(msr->prm,"dnloc",2,&msr->param.dnloc,
                    sizeof(double),"nloc",
                    "<nloc sinc kernel> ");
        msr->param.dnloc2 = 1.0;
        prmAddParam(msr->prm,"dnloc2",2,&msr->param.dnloc2,
                    sizeof(double),"nloc2",
                    "<nloc2 sinc kernel> ");
        msr->param.dnloc3 = 1.0;
        prmAddParam(msr->prm,"dnloc3",2,&msr->param.dnloc3,
                    sizeof(double),"nloc3",
                    "<nloc3 sinc kernel> ");
        msr->param.dalp = 0.0;
        prmAddParam(msr->prm,"dalp",2,&msr->param.dalp,
                    sizeof(double),"alp",
                    "< contribution 2nd kernel> ");
        msr->param.dalp2 = 0.0;
        prmAddParam(msr->prm,"dalp2",2,&msr->param.dalp2,
                    sizeof(double),"alp2",
                    "< contribution 3rd kernel> ");
	msr->param.dalp3 = 0.0;
        prmAddParam(msr->prm,"dalp3",2,&msr->param.dalp3,
                    sizeof(double),"alp3",
                    "< contribution 4rd kernel> ");
	msr->param.dalp4 = 0.0;
        prmAddParam(msr->prm,"dalp4",2,&msr->param.dalp4,
                    sizeof(double),"alp4",
                    "< contribution 5rd kernel> ");
	msr->param.dalp5 = 0.0;
        prmAddParam(msr->prm,"dalp5",2,&msr->param.dalp5,
                    sizeof(double),"alp5",
                    "< contribution 6rd kernel> ");
	msr->param.dalp6 = 0.0;
        prmAddParam(msr->prm,"dalp6",2,&msr->param.dalp6,
                    sizeof(double),"alp6",
                    "< contribution 7rd kernel> ");
	msr->param.dalp7 = 0.0;
        prmAddParam(msr->prm,"dalp7",2,&msr->param.dalp7,
                    sizeof(double),"alp7",
                    "< contribution 8rd kernel> ");
	msr->param.dalp8 = 0.0;
        prmAddParam(msr->prm,"dalp8",2,&msr->param.dalp8,
                    sizeof(double),"alp8",
                    "< contribution 9rd kernel> ");
	msr->param.dalp9 = 0.0;
        prmAddParam(msr->prm,"dalp9",2,&msr->param.dalp9,
                    sizeof(double),"alp9",
                    "< contribution 10rd kernel> ");
	msr->param.dalp10 = 0.0;
        prmAddParam(msr->prm,"dalp10",2,&msr->param.dalp10,
                    sizeof(double),"alp10",
                    "< contribution 11rd kernel> ");
        msr->param.dalp11 = 0.0;
        prmAddParam(msr->prm,"dalp11",2,&msr->param.dalp11,
                    sizeof(double),"alp11",
                    "< contribution 12rd kernel> ");
        msr->param.dalp12 = 0.0;
        prmAddParam(msr->prm,"dalp12",2,&msr->param.dalp12,
                    sizeof(double),"alp12",
                    "< contribution 13rd kernel> ");
	msr->param.dalp13 = 0.0;
        prmAddParam(msr->prm,"dalp13",2,&msr->param.dalp13,
                    sizeof(double),"alp13",
		    "< contribution 14rd kernel> ");
	msr->param.dalp14 = 0.0;
        prmAddParam(msr->prm,"dalp14",2,&msr->param.dalp14,
                    sizeof(double),"alp14",
                    "< contribution 13rd kernel> ");
	msr->param.dalp15 = 0.0;
        prmAddParam(msr->prm,"dalp15",2,&msr->param.dalp15,
                    sizeof(double),"alp15",
                    "< contribution 14rd kernel> ");
	msr->param.dalp16 = 0.0;
        prmAddParam(msr->prm,"dalp16",2,&msr->param.dalp16,
                    sizeof(double),"alp16",
                    "< contribution 15rd kernel> ");
	msr->param.dalp17 = 0.0;
        prmAddParam(msr->prm,"dalp17",2,&msr->param.dalp17,
                    sizeof(double),"alp17",
                    "< contribution 16rd kernel> ");
        msr->param.dalp18 = 0.0;
        prmAddParam(msr->prm,"dalp18",2,&msr->param.dalp18,
                    sizeof(double),"alp18",
                    "< contribution 17rd kernel> ");
        msr->param.dalp19 = 0.0;
        prmAddParam(msr->prm,"dalp19",2,&msr->param.dalp19,
                    sizeof(double),"alp19",
                    "< contribution 18rd kernel> ");
	msr->param.dalp20 = 0.0;
        prmAddParam(msr->prm,"dalp20",2,&msr->param.dalp20,
                    sizeof(double),"alp20",
		    "< contribution 19rd kernel> ");
    	msr->param.dalp21 = 0.0;
        prmAddParam(msr->prm,"dalp21",2,&msr->param.dalp21,
                    sizeof(double),"alp21",
		    "< contribution 20rd kernel> ");
...
/* supernova constants */
	...
	*/
	...
	msr->param.bPatchVerticalGravity = 1;
	prmAddParam(msr->prm,"bPatchVerticalGravity",0,&msr->param.bPatchVerticalGravity,sizeof(int),
				"patch","enable/disable patch vertical gravity = -patchverticalgravity");
...
#ifdef ICGEN
if (prmSpecified(msr->prm, "dPeriod") || prmSpecified(msr->prm, "dxPeriod") || 
    prmSpecified(msr->prm, "dyPeriod") || prmSpecified(msr->prm, "dzPeriod")) {

    FLOAT maxPeriod = 0.0;
    maxPeriod = fmax(maxPeriod, msr->param.dxPeriod);
    maxPeriod = fmax(maxPeriod, msr->param.dyPeriod);
    maxPeriod = fmax(maxPeriod, msr->param.dzPeriod);

    // Set dICR0 to the maximum of these periods
    if (msr->param.bPeriodic) msr->param.dICR0 = 0.99*maxPeriod*msr->param.dICR0;
    printf("ICR0 Beginning %g ",msr->param.dICR0);
 }
 #endif
...
#ifdef SLIDING_PATCH
#ifdef SINKING
#error Do not combine SLIDING_PATCH and SINKING
#endif
// sliding patch requires gravity because the acceleration is modified in pkdGravExternal (See: Quinn+ 2010)
	assert(msr->param.bDoGravity);
	assert(!msr->param.bHeliocentric);
	assert(!msr->param.bFandG);
// The stuff below could work with SLIDING_PATCH in principle but would require checking
// And probably new code in smoothfcn.c
    assert(!msr->param.bDoSinks);
    assert(!msr->param.bBHSink);
    assert(!msr->param.bStarForm);
	if (msr->param.bPatch) {
	  ...
	  if (PP->bRandAzWrap) {
        ...
        msr->param.PP.dOrbFreq = 0;
        msr->param.PP.dOrbFreqZ2 = 0;
...
void msrLogDefines(FILE *fp)
{
...
#endif
#ifdef MHDRESISTIVITY
    fprintf(fp," MHDRESISTIVITY");
#endif
#ifdef MHDCLEANING
    fprintf(fp," MHDCLEANING");
#endif
#ifdef RHOITERATIVE
    fprintf(fp," RHOITERATIVE");
#endif
#ifdef THERMALCONDANI
    fprintf(fp," THERMALCONDANI");
#endif
#ifdef ISPH
    fprintf(fp," ISPH");
#endif
	...
	fprintf(fp," VARALPHA");
#endif
#ifdef CULLENDEHNENRW
	fprintf(fp," CULLENDEHNENRW");
#endif
#ifdef CULLENDEHNEN
...
void msrLogHeader(MSR msr,FILE *fp)
{
    ...
    LogParams(lgr, "NON-INERTIAL REFERENCE FRAME","bPatchVerticalGravity: %d",msr->param.bPatchVerticalGravity);
    ...
    LogParams(lgr, "SPH","nSmooth: %d",msr->param.nSmooth);
		...
		#ifdef MHD
		#ifdef MHDRESISTIVITY
		LogParams(lgr, "SPH","dMHDAlphaB: %g",msr->param.dMHDAlphaB);
		#endif
		#ifdef MHDCLEANING
		LogParams(lgr, "SPH","dMHDPsidecFac: %g",msr->param.dMHDPsidecFac);
		LogParams(lgr, "SPH","dMHDCleanFac: %g",msr->param.dMHDCleanFac);
		#endif
		#endif
...
#endif
#ifdef ICGEN
    LogParams(lgr, "ICGEN","dICR0: %g",msr->param.dICR0);
    LogParams(lgr, "ICGEN","dICRLOOP0: %g",msr->param.dICRLOOP0);
    LogParams(lgr, "ICGEN","dICrhopow: %g",msr->param.dICrhopow);
    LogParams(lgr, "ICGEN","dICdensprofile: %g",msr->param.dICdensprofile);
    LogParams(lgr, "ICGEN","dICdensdir: %g",msr->param.dICdensdir);
    LogParams(lgr, "ICGEN","dICdensouter: %g",msr->param.dICdensouter);
    LogParams(lgr, "ICGEN","dICdensinner: %g",msr->param.dICdensinner);
    LogParams(lgr, "ICGEN","dICdensR: %g",msr->param.dICdensR);
    LogParams(lgr, "ICGEN","dICdensRsmooth: %g",msr->param.dICdensRsmooth);
#endif
#ifdef GASOLINE
    ...
    if (msr->param.bPatch) {
        ...
        LogParams(lgr, "NON-INERTIAL REFERENCE FRAME","dOrbFreqZ: %g",sqrt(PP->dOrbFreqZ2));
    ...
    fprintf(fp, "# Log entries:\n# [dTime] [z] [Etot] [Ekin] [Epot] [Eth] [Emag] [totentrop] [totenstro] [Lx] [Ly] [Lz] [Llinx] [Liny] [Llinz] [cmx] [cmy] [cmz] [MWxy] [MWyz] [MWzx] [RSxy] [RSyz] [RSzx] [divBAvg] [divBMax] [divBerrAvg] [divBerrMax] [alphaAvg] [alphaMax] [betaMax] [betaAvg] [betaMin] [etaresAvg] [kinviscAvg] [rmsmach] [vrms] [rhogasAvg] [rhogasMax] [Q1Avg] [Q1Max] [Q2Avg] [Q2Max] [E0Avg] [E0Max] [Q4Avg] [Q4Max] [WallTime] [dWMax] [dImax] [dEMax] [dMultiEff]\n");
    }
...
double msrReadTipsy(MSR msr)
{
	...
	msr->nGas = h.nsph;
	msr->nStar = h.nstar;
	printf("%d %d %d %d ",msr->N,msr->nDark,msr->nGas,msr->nStar);
...
#endif
#ifdef MHD
    OutputList[(*nOutputList)++]=OUT_MHDX_ARRAY;
		OutputList[(*nOutputList)++]=OUT_MHDY_ARRAY;
		OutputList[(*nOutputList)++]=OUT_MHDZ_ARRAY;
		OutputList[(*nOutputList)++]=OUT_MHDCURLX_ARRAY;
		OutputList[(*nOutputList)++]=OUT_MHDCURLY_ARRAY;
		OutputList[(*nOutputList)++]=OUT_MHDCURLZ_ARRAY;
		OutputList[(*nOutputList)++]=OUT_MHDDIV_ARRAY;
#endif
#ifdef CULLENDEHNEN
		OutputList[(*nOutputList)++]=OUT_AX_ARRAY;
                OutputList[(*nOutputList)++]=OUT_AY_ARRAY;
                OutputList[(*nOutputList)++]=OUT_AZ_ARRAY;
                OutputList[(*nOutputList)++]=OUT_GX_ARRAY;
                OutputList[(*nOutputList)++]=OUT_GY_ARRAY;
                OutputList[(*nOutputList)++]=OUT_GZ_ARRAY;
#endif
#ifdef CHECKQUALITY
    OutputList[(*nOutputList)++]=OUT_Q1_ARRAY;
    OutputList[(*nOutputList)++]=OUT_Q2_ARRAY;
    OutputList[(*nOutputList)++]=OUT_E0_ARRAY;
    OutputList[(*nOutputList)++]=OUT_Q4_ARRAY;
#endif
#ifdef THERMALCONDANI
		OutputList[(*nOutputList)++]=OUT_UDOTANICOND_ARRAY;
		OutputList[(*nOutputList)++]=OUT_GRADTX_ARRAY;
    OutputList[(*nOutputList)++]=OUT_GRADTY_ARRAY;
    OutputList[(*nOutputList)++]=OUT_GRADTZ_ARRAY;
#endif
#ifdef UNONCOOL
...
#ifdef CD_DEBUG
    OutputList[(*nOutputList)++]=OUT_DVXDX_ARRAY;
    OutputList[(*nOutputList)++]=OUT_DVYDX_ARRAY;
    OutputList[(*nOutputList)++]=OUT_DVZDX_ARRAY;
    OutputList[(*nOutputList)++]=OUT_DVXDY_ARRAY;
    OutputList[(*nOutputList)++]=OUT_DVYDY_ARRAY;
    OutputList[(*nOutputList)++]=OUT_DVZDY_ARRAY;
    OutputList[(*nOutputList)++]=OUT_DVXDZ_ARRAY;
    OutputList[(*nOutputList)++]=OUT_DVYDZ_ARRAY;
    OutputList[(*nOutputList)++]=OUT_DVZDZ_ARRAY;
#endif
    if (!msr->param.bBulkViscosity){
...
void msrCreateOutputList(MSR msr, int (*nOutputList), int OutputList[])
    ...
    OutputList[(*nOutputList)++]=OUT_MHDX_ARRAY;
	  ...
	  OutputList[(*nOutputList)++]=OUT_MHDZ_ARRAY;
		...
		OutputList[(*nOutputList)++]=OUT_MHDCURLZ_ARRAY;
		OutputList[(*nOutputList)++]=OUT_MHDDIV_ARRAY;
		OutputList[(*nOutputList)++]=OUT_MHDUDOTBDISS_ARRAY;
		OutputList[(*nOutputList)++]=OUT_MHDBDOTRHO_ARRAY;
		OutputList[(*nOutputList)++]=OUT_MHDBDOTDISS_ARRAY;
		OutputList[(*nOutputList)++]=OUT_MHDBDOTCLEAN_ARRAY;
		OutputList[(*nOutputList)++]=OUT_MHDBCLEAN_ARRAY;
#endif
#ifdef GETVISCPARA
		OutputList[(*nOutputList)++]=OUT_VISCCOEF_ARRAY;
#ifdef MHD
		OutputList[(*nOutputList)++]=OUT_RESICOEF_ARRAY;
#endif
#endif
#ifdef CULLENDEHNEN
                OutputList[(*nOutputList)++]=OUT_AX_ARRAY;
...
void msrSmoothFcnParam(MSR msr, double dTime, SMF *psmf)
    ...
    psmf->bGasIsothermal = msr->param.bGasIsothermal;
...
#ifdef MHD
#ifdef MHDRESISTIVITY
	psmf->alphaB=msr->param.dMHDAlphaB;
#endif
#ifdef MHDCLEANING
...
psmf->fch=msr->param.dMHDCleanFac;
 ...
 psmf->alp5 = msr->param.dalp5;
 psmf->alp6 = msr->param.dalp6;
 psmf->alp7 = msr->param.dalp7;
 psmf->alp8 = msr->param.dalp8;
 psmf->alp9 = msr->param.dalp9;
 psmf->alp10 = msr->param.dalp10;
 psmf->alp11 = msr->param.dalp11;
 psmf->alp12 = msr->param.dalp12;
 psmf->alp13 = msr->param.dalp13;
 psmf->alp14 = msr->param.dalp14;
 psmf->alp15 = msr->param.dalp15;
 psmf->alp16 = msr->param.dalp16;
 psmf->alp17 = msr->param.dalp17;
 psmf->alp18 = msr->param.dalp18;
 psmf->alp19 = msr->param.dalp19;
 psmf->alp20 = msr->param.dalp20;
 psmf->alp21 = msr->param.dalp21;
 psmf->nloc = msr->param.dnloc;
 psmf->nloc2 = msr->param.dnloc2;
 psmf->nloc3 = msr->param.dnloc3;
 //psmf->Wzero = 1.0;
 //psmf->Wzero = msr->param.dnloc;
 //psmf->Wzero2 = msr->param.dnloc2;
 //psmf->Wzero3 = msr->param.dnloc3;
 psmf->Wzero = 1.0;
 psmf->Wzero2 = 1.0;
 psmf->Wzero3 = 1.0;
 psmf->Wzero4 = 1.0;
 ...
 if(nSmooth<=16){
   //psmf->Wzero = (0.78925781-0.50)/(14)*(nSmooth-2)+0.50;
   psmf->Wzero = (0.82890625-0.50)/(14)*(nSmooth-2)+0.50;
   psmf->Wzero2 = (0.85625-0.50)/(14)*(nSmooth-2)+0.50;
   psmf->Wzero3 = (0.92216797-0.50)/(14)*(nSmooth-2)+0.50;
   psmf->Wzero4 = (0.91865234-0.50)/(14)*(nSmooth-2)+0.50;
   psmf->Wzero5 = (0.90156097-0.50)/(14)*(nSmooth-2)+0.50;
   psmf->Wzero6 = (0.8734375-0.50)/(14)*(nSmooth-2)+0.50;
   psmf->Wzero7 = (0.94551392-0.50)/(14)*(nSmooth-2)+0.50;
   psmf->Wzero8 = (0.7812439-0.50)/(14)*(nSmooth-2)+0.50;
   psmf->Wzero9 = (0.74023437-0.50)/(14)*(nSmooth-2)+0.50;
   psmf->Wzero10 = (0.80507812-0.50)/(14)*(nSmooth-2)+0.50;
   psmf->Wzero11 = (0.70195312-0.50)/(14)*(nSmooth-2)+0.50;
   psmf->Wzero12 = (0.54921875-0.40)/(14)*(nSmooth-2)+0.40;
   psmf->Wzero13 = (0.43124695-0.40)/(14)*(nSmooth-2)+0.40;
   psmf->Wzero14 = (0.78925781-0.50)/(14)*(nSmooth-2)+0.50;

   psmf->Wzero15 = (0.7765625-0.50)/(14)*(nSmooth-2)+0.50;
   psmf->Wzero16 = (0.67424316-0.50)/(14)*(nSmooth-2)+0.50;
   //psmf->Wzero16 = (0.69399414-0.50)/(14)*(nSmooth-2)+0.50;
   psmf->Wzero17 = (0.83125305-0.50)/(14)*(nSmooth-2)+0.50;
   psmf->Wzero18 = (0.74882812-0.50)/(14)*(nSmooth-2)+0.50;
   psmf->Wzero19 = (0.83544617-0.50)/(14)*(nSmooth-2)+0.50;
   psmf->Wzero20 = (0.80136719-0.50)/(14)*(nSmooth-2)+0.50;
   psmf->Wzero21 = (0.84145508-0.50)/(14)*(nSmooth-2)+0.50;
   psmf->Wzero22 = (0.81279297-0.50)/(14)*(nSmooth-2)+0.50;
   //psmf->Wzero22 = (0.84213867-0.50)/(14)*(nSmooth-2)+0.50;

 }
 else if(nSmooth<=24){
   psmf->Wzero = (0.87822266-0.82890625)/8*(nSmooth-16)+0.82890625;
   //psmf->Wzero = (0.96679688-0.78925781)/8*(nSmooth-16)+0.78925781;
   psmf->Wzero2 = (0.92011719-0.85625)/8*(nSmooth-16)+0.85625;

   psmf->Wzero3 = (0.9765625-0.92216797)/8*(nSmooth-16)+0.92216797;

   psmf->Wzero4 = (0.96523437-0.91865234)/8*(nSmooth-16)+0.91865234;
   psmf->Wzero5 = (0.95498047-0.90156097)/8*(nSmooth-16)+0.90156097;
   psmf->Wzero6 = (0.89990234-0.8734375)/8*(nSmooth-16)+0.8734375;
   psmf->Wzero7 = (0.97011719-0.94551392)/8*(nSmooth-16)+0.94551392;
   psmf->Wzero8 = (0.86464844-0.7812439)/8*(nSmooth-16)+0.7812439;
   psmf->Wzero9 = (0.80107422-0.74023437)/8*(nSmooth-16)+0.74023437;
   psmf->Wzero10 = (0.90039062-0.80507812)/8*(nSmooth-16)+0.80507812;
   psmf->Wzero11 = (0.87412109-0.70195312)/8*(nSmooth-16)+0.70195312;
   psmf->Wzero12 = (0.76171875-0.54921875)/8*(nSmooth-16)+0.54921875;
   psmf->Wzero13 = (0.63945312-0.43124695)/8*(nSmooth-16)+0.43124695;
   psmf->Wzero14 = (0.96679688-0.78925781)/8*(nSmooth-16)+0.78925781;

   psmf->Wzero15 = (0.85302734-0.7765625)/8*(nSmooth-16)+0.7765625;
   psmf->Wzero16 = (0.85517578-0.67424316)/8*(nSmooth-16)+0.67424316;
   //psmf->Wzero16 = (0.82734375-0.69399414)/8*(nSmooth-16)+0.69399414;
   psmf->Wzero17 = (0.93046875-0.83125305)/8*(nSmooth-16)+0.83125305;
   psmf->Wzero18 = (0.85976562-0.74882812)/8*(nSmooth-16)+0.74882812;
   psmf->Wzero19 = (0.95097656-0.83544617)/8*(nSmooth-16)+0.83544617;
   psmf->Wzero20 = (0.90957031-0.80136719)/8*(nSmooth-16)+0.80136719;
   psmf->Wzero21 = (0.95-0.84145508)/8*(nSmooth-16)+0.84145508;
   psmf->Wzero22 = (0.91591797-0.81279297)/8*(nSmooth-16)+0.81279297;
   //psmf->Wzero22 = (0.96640625-0.84213867)/8*(nSmooth-16)+0.84213867;

 }
 else if(nSmooth<=32){
   psmf->Wzero = (0.88066406-0.87822266)/8*(nSmooth-24)+0.87822266;
   //psmf->Wzero = (0.98798828-0.96679688)/8*(nSmooth-24)+0.96679688;
   psmf->Wzero2 = (0.92617187-0.92011719)/8*(nSmooth-24)+0.92011719;
   psmf->Wzero3 = (0.98125-0.9765625)/8*(nSmooth-24)+0.9765625;

   psmf->Wzero4 = (0.971875-0.96523437)/8*(nSmooth-24)+0.96523437;
   psmf->Wzero5 = (0.96308594-0.95498047)/8*(nSmooth-24)+0.95498047;
   psmf->Wzero6 = (0.91533203-0.89990234)/8*(nSmooth-24)+0.89990234;
   psmf->Wzero7 = (0.98007812-0.97011719)/8*(nSmooth-24)+0.97011719;
   psmf->Wzero8 = (0.88203125-0.86464844)/8*(nSmooth-24)+0.86464844;
   psmf->Wzero9 = (0.82080078-0.80107422)/8*(nSmooth-24)+0.80107422;
   psmf->Wzero10 = (0.91347656-0.90039062)/8*(nSmooth-24)+0.90039062;
   psmf->Wzero11 = (0.91611328-0.87412109)/8*(nSmooth-24)+0.87412109;
   psmf->Wzero12 = (0.83505859-0.76171875)/8*(nSmooth-24)+0.76171875;
   psmf->Wzero13 = (0.72568359-0.63945312)/8*(nSmooth-24)+0.63945312;
   psmf->Wzero14 = (0.98798828-0.96679688)/8*(nSmooth-24)+0.96679688;

   psmf->Wzero15 = (0.86884766-0.85302734)/8*(nSmooth-24)+0.85302734;
   psmf->Wzero16 = (0.88662109-0.85517578)/8*(nSmooth-24)+0.85517578;
  //   psmf->Wzero16 = (0.84921875-0.82734375)/8*(nSmooth-24)+0.82734375;
   psmf->Wzero17 = (0.93710937-0.93046875)/8*(nSmooth-24)+0.93046875;
   psmf->Wzero18 = (0.87744141-0.85976562)/8*(nSmooth-24)+0.85976562;
   psmf->Wzero19 = (0.95742187-0.95097656)/8*(nSmooth-24)+0.95097656;
   psmf->Wzero20 = (0.92109375-0.90957031)/8*(nSmooth-24)+0.90957031;
   psmf->Wzero21 = (0.97675781-0.95)/8*(nSmooth-24)+0.95;
   psmf->Wzero22 = (0.92558594-0.91591797)/8*(nSmooth-24)+0.91591797;
     // psmf->Wzero22 = (0.97558594-0.96640625)/8*(nSmooth-24)+0.96640625;
 }
 else if(nSmooth<=48){
   psmf->Wzero = (0.89130859-0.88066406)/16*(nSmooth-32)+0.88066406;
 //  psmf->Wzero = (0.99375-0.98798828)/16*(nSmooth-32)+0.98798828;
   psmf->Wzero2 = (0.93398437-0.92617187)/16*(nSmooth-32)+0.92617187;
   psmf->Wzero3 = (0.98671875-0.98125)/16*(nSmooth-32)+0.98125;

   psmf->Wzero4 = (0.97919922-0.971875)/16*(nSmooth-32)+0.971875;
   psmf->Wzero5 = (0.97421875-0.96308594)/16*(nSmooth-32)+0.96308594;
   psmf->Wzero6 = (0.94384766-0.91533203)/16*(nSmooth-32)+0.91533203;
   psmf->Wzero7 = (0.98652344-0.98007812)/16*(nSmooth-32)+0.98007812;
   psmf->Wzero8 = (0.90712891-0.88203125)/16*(nSmooth-32)+0.88203125;
   psmf->Wzero9 = (0.85361328-0.82080078)/16*(nSmooth-32)+0.82080078;
   psmf->Wzero10 = (0.93349609-0.91347656)/16*(nSmooth-32)+0.91347656;
   psmf->Wzero11 = (0.95019531-0.91611328)/16*(nSmooth-32)+0.91611328;
   psmf->Wzero12 = (0.92001953-0.83505859)/16*(nSmooth-32)+0.83505859;
   psmf->Wzero13 = (0.85019531-0.72568359)/16*(nSmooth-32)+0.72568359;
   psmf->Wzero14 = (0.99375-0.98798828)/16*(nSmooth-32)+0.98798828;

   psmf->Wzero15 = (0.89765625-0.86884766)/16*(nSmooth-32)+0.86884766;
   psmf->Wzero16 = (0.88574219-0.88662109)/16*(nSmooth-32)+0.88662109;
 //  psmf->Wzero16 = (0.84960938-0.84921875)/16*(nSmooth-32)+0.84921875;
   psmf->Wzero17 = (0.94462891-0.93710937)/16*(nSmooth-32)+0.93710937;
   psmf->Wzero18 = (0.90087891-0.87744141)/16*(nSmooth-32)+0.87744141;
   psmf->Wzero19 = (0.96210937-0.95742187)/16*(nSmooth-32)+0.95742187;
   psmf->Wzero20 = (0.93486328-0.92109375)/16*(nSmooth-32)+0.92109375;
   psmf->Wzero21 = (0.98193359-0.97675781)/16*(nSmooth-32)+0.97675781;
   psmf->Wzero22 = (0.93730469-0.92558594)/16*(nSmooth-32)+0.92558594;
   //  psmf->Wzero22 = (0.98154297-0.97558594)/16*(nSmooth-32)+0.97558594;
 }
 else if(nSmooth<=64){
   psmf->Wzero = (0.89912109-0.89130859)/16*(nSmooth-48)+0.89130859;
//      psmf->Wzero = (0.99619141-0.99375)/16*(nSmooth-48)+0.99375;
   psmf->Wzero2 = (0.94042969-0.93398437)/16*(nSmooth-48)+0.93398437;
   psmf->Wzero3 = (0.99033203-0.98671875)/16*(nSmooth-48)+0.98671875;

   psmf->Wzero4 = (0.98515625-0.97919922)/16*(nSmooth-48)+0.97919922;
   psmf->Wzero5 = (0.98632812-0.97421875)/16*(nSmooth-48)+0.97421875;
   psmf->Wzero6 = (0.95839844-0.94384766)/16*(nSmooth-48)+0.94384766;
   psmf->Wzero7 = (0.98964844-0.98652344)/16*(nSmooth-48)+0.98652344;
   psmf->Wzero8 = (0.93808594-0.90712891)/16*(nSmooth-48)+0.90712891;
   psmf->Wzero9 = (0.8921875-0.85361328)/16*(nSmooth-48)+0.85361328;
   psmf->Wzero10 = (0.95693359-0.93349609)/16*(nSmooth-48)+0.93349609;
   psmf->Wzero11 = (0.96748047-0.95019531)/16*(nSmooth-48)+0.95019531;
   psmf->Wzero12 = (0.96542969-0.92001953)/16*(nSmooth-48)+0.92001953;
   psmf->Wzero13 = (0.93779297-0.85019531)/16*(nSmooth-48)+0.85019531;
   psmf->Wzero14 = (0.99619141-0.99375)/16*(nSmooth-48)+0.99375;

   psmf->Wzero15 = (0.92294922-0.89765625)/16*(nSmooth-48)+0.89765625;
   psmf->Wzero16 = (0.92255859-0.88574219)/16*(nSmooth-48)+0.88574219;
//    psmf->Wzero16 = (0.87119141-0.84960938)/16*(nSmooth-48)+0.84960938;
   psmf->Wzero17 = (0.95146484-0.94462891)/16*(nSmooth-48)+0.94462891;
   psmf->Wzero18 = (0.91757812-0.90087891)/16*(nSmooth-48)+0.90087891;
   psmf->Wzero19 = (0.96582031-0.96210937)/16*(nSmooth-48)+0.96210937;
   psmf->Wzero20 = (0.94765625-0.93486328)/16*(nSmooth-48)+0.93486328;
   psmf->Wzero21 = (0.98554688-0.98193359)/16*(nSmooth-48)+0.98193359;
   psmf->Wzero22 = (0.94824219-0.93730469)/16*(nSmooth-48)+0.93730469;
 //   psmf->Wzero22 = (0.98662109-0.98154297)/16*(nSmooth-48)+0.98154297;
 }
 else if(nSmooth<=96){
   psmf->Wzero = (0.91503906-0.89912109)/32*(nSmooth-64)+0.89912109;
//      psmf->Wzero = (0.99775391-0.99619141)/32*(nSmooth-64)+0.99619141;
   psmf->Wzero2 = (0.94951172-0.94042969)/32*(nSmooth-64)+0.94042969;
   psmf->Wzero3 = (0.99414062-0.99033203)/32*(nSmooth-64)+0.99033203;

   psmf->Wzero4 = (0.99130859-0.98515625)/32*(nSmooth-64)+0.98515625;
   psmf->Wzero5 = (0.99189453-0.98632812)/32*(nSmooth-64)+0.98632812;
   psmf->Wzero6 = (0.97167969-0.95839844)/32*(nSmooth-64)+0.95839844;
   psmf->Wzero7 = (0.99326172-0.98964844)/32*(nSmooth-64)+0.98964844;
   psmf->Wzero8 = (0.95830078-0.93808594)/32*(nSmooth-64)+0.93808594;
   psmf->Wzero9 = (0.91513672-0.8921875)/32*(nSmooth-64)+0.8921875;
   psmf->Wzero10 = (0.97158203-0.95693359)/32*(nSmooth-64)+0.95693359;
   psmf->Wzero11 = (0.98417969-0.96748047)/32*(nSmooth-64)+0.96748047;
   psmf->Wzero12 = (0.98515625-0.96542969)/32*(nSmooth-64)+0.96542969;
   psmf->Wzero13 = (0.98095703-0.93779297)/32*(nSmooth-64)+0.93779297;
   psmf->Wzero14 = (0.99775391-0.99619141)/32*(nSmooth-64)+0.99619141;

   psmf->Wzero15 = (0.93652344-0.92294922)/32*(nSmooth-64)+0.92294922;
   psmf->Wzero16 = (0.95058594-0.92255859)/32*(nSmooth-64)+0.92255859;
//   psmf->Wzero16 = (0.8875-0.87119141)/32*(nSmooth-64)+0.87119141;
   psmf->Wzero17 = (0.96054687-0.95146484)/32*(nSmooth-64)+0.95146484;
   psmf->Wzero18 = (0.93232422-0.91757812)/32*(nSmooth-64)+0.91757812;
   psmf->Wzero19 = (0.97236328-0.96582031)/32*(nSmooth-64)+0.96582031;
   psmf->Wzero20 = (0.96025391-0.94765625)/32*(nSmooth-64)+0.94765625;
   psmf->Wzero21 = (0.99023438-0.98554688)/32*(nSmooth-64)+0.98554688;
   psmf->Wzero22 = (0.95908203-0.94824219)/32*(nSmooth-64)+0.94824219;
//     psmf->Wzero22 = (0.99111328-0.98662109)/32*(nSmooth-64)+0.98662109;
 }
 else if(nSmooth<=128){
   psmf->Wzero = (0.93125-0.91503906)/32*(nSmooth-96)+0.91503906;
//      psmf->Wzero = (0.99863281-0.99775391)/32*(nSmooth-96)+0.99775391;
   psmf->Wzero2 = (0.95800781-0.94951172)/32*(nSmooth-96)+0.94951172;
   psmf->Wzero3 = (0.99541016-0.99414062)/32*(nSmooth-96)+0.99414062;

   psmf->Wzero4 = (0.99306641-0.99130859)/32*(nSmooth-96)+0.99130859;
   psmf->Wzero5 = (0.99355469-0.99189453)/32*(nSmooth-96)+0.99189453;
   psmf->Wzero6 = (0.97919922-0.97167969)/32*(nSmooth-96)+0.97167969;
   psmf->Wzero7 = (0.99482422-0.99326172)/32*(nSmooth-96)+0.99326172;
   psmf->Wzero8 = (0.96572266-0.95830078)/32*(nSmooth-96)+0.95830078;
   psmf->Wzero9 = (0.92734375-0.91513672)/32*(nSmooth-96)+0.91513672;
   psmf->Wzero10 = (0.97675781-0.97158203)/32*(nSmooth-96)+0.97158203;
   psmf->Wzero11 = (0.99013672-0.98417969)/32*(nSmooth-96)+0.98417969;
   psmf->Wzero12 = (0.99130859-0.98515625)/32*(nSmooth-96)+0.98515625;
   psmf->Wzero13 = (0.99003906-0.98095703)/32*(nSmooth-96)+0.98095703;
   psmf->Wzero14 = (0.99863281-0.99775391)/32*(nSmooth-96)+0.99775391;

   psmf->Wzero15 = (0.94716797-0.93652344)/32*(nSmooth-96)+0.93652344;
   psmf->Wzero16 = (0.97558594-0.95058594)/32*(nSmooth-96)+0.95058594;
    //  psmf->Wzero16 = (0.89882812-0.8875)/32*(nSmooth-96)+0.8875;
   psmf->Wzero17 = (0.96875-0.96054687)/32*(nSmooth-96)+0.96054687;
   psmf->Wzero18 = (0.94550781-0.93232422)/32*(nSmooth-96)+0.93232422;
   psmf->Wzero19 = (0.97832031-0.97236328)/32*(nSmooth-96)+0.97236328;
   psmf->Wzero20 = (0.97089844-0.96025391)/32*(nSmooth-96)+0.96025391;
   psmf->Wzero21 = (0.99355469-0.99023438)/32*(nSmooth-96)+0.99023438;
   psmf->Wzero22 = (0.96845703-0.95908203)/32*(nSmooth-96)+0.95908203;
  // psmf->Wzero22 = (0.99306641-0.99111328)/32*(nSmooth-96)+0.99111328;


 }
 else if(nSmooth<=192){
   psmf->Wzero = (0.95351562-0.93125)/64*(nSmooth-128)+0.93125;
//      psmf->Wzero = (0.99921875-0.99863281)/64*(nSmooth-128)+0.99863281;
   psmf->Wzero2 = (0.96953125-0.95800781)/64*(nSmooth-128)+0.95800781; //seems off at N192
   psmf->Wzero3 = (0.99677734-0.99541016)/64*(nSmooth-128)+0.99541016;

   psmf->Wzero4 = (0.9953125-0.99306641)/64*(nSmooth-128)+0.99306641;
   psmf->Wzero5 = (0.99521484-0.99355469)/64*(nSmooth-128)+0.99355469;
   psmf->Wzero6 = (0.98808594-0.97919922)/64*(nSmooth-128)+0.97919922;
   psmf->Wzero7 = (0.99628906-0.99482422)/64*(nSmooth-128)+0.99482422;
   psmf->Wzero8 = (0.97832031-0.96572266)/64*(nSmooth-128)+0.96572266;
   psmf->Wzero9 = (0.94716797-0.92734375)/64*(nSmooth-128)+0.92734375;
   psmf->Wzero10 = (0.98554688-0.97675781)/64*(nSmooth-128)+0.97675781;
   psmf->Wzero11 = (0.99443359-0.99013672)/64*(nSmooth-128)+0.99013672;
   psmf->Wzero12 = (0.99677734-0.99130859)/64*(nSmooth-128)+0.99130859;
   psmf->Wzero13 = (0.99677734-0.99003906)/64*(nSmooth-128)+0.99003906;
   psmf->Wzero14 = (0.99921875-0.99863281)/64*(nSmooth-128)+0.99863281;

   psmf->Wzero15 = (0.96083984-0.94716797)/64*(nSmooth-128)+0.94716797;
   psmf->Wzero16 = (1.01279297-0.97558594)/64*(nSmooth-128)+0.97558594;
//     psmf->Wzero16 = (0.91640625-0.89882812)/64*(nSmooth-128)+0.89882812;
   psmf->Wzero17 = (0.97900391-0.96875)/64*(nSmooth-128)+0.96875;
   psmf->Wzero18 = (0.96123047-0.94550781)/64*(nSmooth-128)+0.94550781;
   psmf->Wzero19 = (0.98623047-0.97832031)/64*(nSmooth-128)+0.97832031;
   psmf->Wzero20 = (0.98203125-0.97089844)/64*(nSmooth-128)+0.97089844;
   psmf->Wzero21 = (0.99746094-0.99355469)/64*(nSmooth-128)+0.99355469;
   psmf->Wzero22 = (0.97871094-0.96845703)/64*(nSmooth-128)+0.96845703;
//      psmf->Wzero22 = (0.99589844-0.99306641)/64*(nSmooth-128)+0.99306641;

 }
 else if(nSmooth<=256){
   psmf->Wzero = (0.9625-0.95351562)/64*(nSmooth-192)+0.95351562;
//      psmf->Wzero = (0.99941406-0.99921875)/64*(nSmooth-192)+0.99921875;
   psmf->Wzero2 = (0.97480469-0.96953125)/64*(nSmooth-192)+0.96953125;
   psmf->Wzero3 = (0.99746094-0.99677734)/64*(nSmooth-192)+0.99677734;

   psmf->Wzero4 = (0.99638672-0.9953125)/64*(nSmooth-192)+0.9953125;
   psmf->Wzero5 = (0.99609375-0.99521484)/64*(nSmooth-192)+0.99521484;
   psmf->Wzero6 = (0.99277344-0.98808594)/64*(nSmooth-192)+0.98808594;
	psmf->Wzero7 = (0.99707031-0.99628906)/64*(nSmooth-192)+0.99628906;
	psmf->Wzero8 = (0.98271484-0.97832031)/64*(nSmooth-192)+0.97832031;
	psmf->Wzero9 = (0.95458984-0.94716797)/64*(nSmooth-192)+0.94716797;
   psmf->Wzero10 = (0.98847656-0.98554688)/64*(nSmooth-192)+0.98554688;
   	psmf->Wzero11 = (0.99658203-0.99443359)/64*(nSmooth-192)+0.99443359;
	psmf->Wzero12 = (0.99814453-0.99677734)/64*(nSmooth-192)+0.99677734;
	psmf->Wzero13 = (0.9984375-0.99677734)/64*(nSmooth-192)+0.99677734;
	psmf->Wzero14 = (0.99941406-0.99921875)/64*(nSmooth-192)+0.99921875;

	psmf->Wzero15 = (0.96826172-0.96083984)/64*(nSmooth-192)+0.96083984;
	psmf->Wzero16 = (1.04570312-1.01279297)/64*(nSmooth-192)+1.01279297;
//	psmf->Wzero16 = (0.92861328-0.91640625)/64*(nSmooth-192)+0.91640625;
	psmf->Wzero17 = (0.9828125-0.97900391)/64*(nSmooth-192)+0.97900391;
	psmf->Wzero18 = (0.96835938-0.96123047)/64*(nSmooth-192)+0.96123047;
	psmf->Wzero19 = (0.98867188-0.98623047)/64*(nSmooth-192)+0.98623047;
	psmf->Wzero20 = (0.98574219-0.98203125)/64*(nSmooth-192)+0.98203125;
	psmf->Wzero21 = (0.99814453-0.99746094)/64*(nSmooth-192)+0.99746094;
	psmf->Wzero22 = (0.9828125-0.97871094)/64*(nSmooth-192)+0.97871094;
//	psmf->Wzero22 = (0.99667969-0.99589844)/64*(nSmooth-192)+0.99589844;

 }
 else if(nSmooth<=512){
   psmf->Wzero = (0.98095703-0.9625)/256*(nSmooth-256)+0.9625;
//      psmf->Wzero = (0.99960938-0.99941406)/256*(nSmooth-256)+0.99941406;
   psmf->Wzero2 = (0.98603516-0.97480469)/256*(nSmooth-256)+0.97480469;
   psmf->Wzero3 = (0.99853516-0.99746094)/256*(nSmooth-256)+0.99746094;

   psmf->Wzero4 = (0.99794922-0.99638672)/256*(nSmooth-256)+0.99638672;
   psmf->Wzero5 = (0.99755859-0.99609375)/256*(nSmooth-256)+0.99609375;
   psmf->Wzero6 = (1.00058594-0.99277344)/256*(nSmooth-256)+0.99277344;
   psmf->Wzero7 = (0.99824219-0.99707031)/256*(nSmooth-256)+0.99707031;
   psmf->Wzero8 = (0.99082031-0.98271484)/256*(nSmooth-256)+0.98271484;
   psmf->Wzero9 = (0.97119141-0.95458984)/256*(nSmooth-256)+0.95458984;
   psmf->Wzero10 = (0.99394531-0.98847656)/256*(nSmooth-256)+0.98847656;
   psmf->Wzero11 = (0.99873047-0.99658203)/256*(nSmooth-256)+0.99658203;
   psmf->Wzero12 = (0.99941406-0.99814453)/256*(nSmooth-256)+0.99814453;
   psmf->Wzero13 = (0.99960938-0.9984375)/256*(nSmooth-256)+0.9984375;
   psmf->Wzero14 = (0.99960938-0.99941406)/256*(nSmooth-256)+0.99941406;

   psmf->Wzero15 = (0.98261719-0.96826172)/256*(nSmooth-256)+0.96826172;
   psmf->Wzero16 = (1.14082031-1.04570312)/256*(nSmooth-256)+1.04570312;
//   psmf->Wzero16 = (0.95078125-0.92861328)/256*(nSmooth-256)+0.92861328;
   psmf->Wzero17 = (0.99072266-0.9828125)/256*(nSmooth-256)+0.9828125;
   psmf->Wzero18 = (0.9828125-0.96835938)/256*(nSmooth-256)+0.96835938;
   psmf->Wzero19 = (0.99404297-0.98867188)/256*(nSmooth-256)+0.98867188;
   psmf->Wzero20 = (0.99296875-0.98574219)/256*(nSmooth-256)+0.98574219;
   psmf->Wzero21 = (0.99931641-0.99814453)/256*(nSmooth-256)+0.99814453;
   psmf->Wzero22 = (0.99091797-0.9828125)/256*(nSmooth-256)+0.9828125;
  // psmf->Wzero22 = (0.99824219-0.99667969)/256*(nSmooth-256)+0.99667969;
}

 #ifdef ISPH2
if(nSmooth <= 16){
    psmf->Wzero *= 0.98046875; // from WOPT24Dwisph.txt
    psmf->Wzero2 *= 0.975; // from WOPT32Dwisph.txt
    psmf->Wzero3 *= 0.9687561; // from WOPT64Dwisph.txt
    psmf->Wzero4 *= 0.97070312; // from WOPT128Dwisph.txt
    psmf->Wzero5 *= 0.9703125; // from WOPT256Dwisph.txt
    psmf->Wzero6 *= 0.98066406; // from WOPTCM04wisph.txt
    psmf->Wzero7 *= 0.97753906; // from WOPTWuC2wisph.txt
    psmf->Wzero8 *= 0.96992188; // from WOPTBUHwisph.txt
    psmf->Wzero9 *= 0.97553711; // from WOPTCM05wisph.txt
    psmf->Wzero10 *= 1.00546875; // from WOPTC2wisph.txt
    psmf->Wzero11 *= 0.9625; // from WOPTC4wisph.txt
    psmf->Wzero12 *= 0.9734375; // from WOPTC6wisph.txt
    psmf->Wzero13 *= 0.97441406; // from WOPTC8wisph.txt
    psmf->Wzero14 *= 1.00283203; // from WOPTWuC4wisph.txt
    psmf->Wzero15 *= 1.007168; // from WOPT32GRDwisphNEW.txt
    psmf->Wzero16 *= 1.018503; // from WOPT32GRDISPHwisphNEW.txt
    psmf->Wzero17 *= 1.005165; // from WOPT64GRDwisphNEW.txt
    psmf->Wzero18 *= 1.006781; // from WOPT64GRDISPHwisphNEW.txt
    psmf->Wzero19 *= 1.004387; // from WOPT128GRDwisphNEW.txt
    psmf->Wzero20 *= 1.004874; // from WOPT128GRDISPHwisphNEW.txt
    psmf->Wzero21 *= 1.004004; // from WOPT256GRDwisphNEW.txt
    psmf->Wzero22 *= 1.005437; // from WOPT256GRDISPHwisphNEW.txt
}
else if(nSmooth <= 24){
    psmf->Wzero *= 0.99960938; // from WOPT24Dwisph.txt
    psmf->Wzero2 *= 0.99990234; // from WOPT32Dwisph.txt
    psmf->Wzero3 *= 0.99609375; // from WOPT64Dwisph.txt
    psmf->Wzero4 *= 0.99609375; // from WOPT128Dwisph.txt
    psmf->Wzero5 *= 0.99609375; // from WOPT256Dwisph.txt
    psmf->Wzero6 *= 0.97480469; // from WOPTCM04wisph.txt
    psmf->Wzero7 *= 0.98984375; // from WOPTWuC2wisph.txt
    psmf->Wzero8 *= 0.99638672; // from WOPTBUHwisph.txt
    psmf->Wzero9 *= 0.99980469; // from WOPTCM05wisph.txt
    psmf->Wzero10 *= 0.99667969; // from WOPTC2wisph.txt
    psmf->Wzero11 *= 0.99736328; // from WOPTC4wisph.txt
    psmf->Wzero12 *= 0.99687805; // from WOPTC6wisph.txt
    psmf->Wzero13 *= 0.99804688; // from WOPTC8wisph.txt
    psmf->Wzero14 *= 0.9984375; // from WOPTWuC4wisph.txt
    psmf->Wzero15 *= 1.002862; // from WOPT32GRDwisphNEW.txt
    psmf->Wzero16 *= 0.979331; // from WOPT32GRDISPHwisphNEW.txt
    psmf->Wzero17 *= 0.999370; // from WOPT64GRDwisphNEW.txt
    psmf->Wzero18 *= 1.001477; // from WOPT64GRDISPHwisphNEW.txt
    psmf->Wzero19 *= 0.998152; // from WOPT128GRDwisphNEW.txt
    psmf->Wzero20 *= 0.999893; // from WOPT128GRDISPHwisphNEW.txt
    psmf->Wzero21 *= 1.017064; // from WOPT256GRDwisphNEW.txt
    psmf->Wzero22 *= 0.999893; // from WOPT256GRDISPHwisphNEW.txt
}
else if(nSmooth <= 32){
    psmf->Wzero *= 0.99775391; // from WOPT24Dwisph.txt
    psmf->Wzero2 *= 0.99980469; // from WOPT32Dwisph.txt
    psmf->Wzero3 *= 0.99111328; // from WOPT64Dwisph.txt
    psmf->Wzero4 *= 0.990625; // from WOPT128Dwisph.txt
    psmf->Wzero5 *= 0.99355469; // from WOPT256Dwisph.txt
    psmf->Wzero6 *= 0.90979004; // from WOPTCM04wisph.txt
    psmf->Wzero7 *= 0.98671875; // from WOPTWuC2wisph.txt
    psmf->Wzero8 *= 0.99716797; // from WOPTBUHwisph.txt
    psmf->Wzero9 *= 1.00048828; // from WOPTCM05wisph.txt
    psmf->Wzero10 *= 0.99541016; // from WOPTC2wisph.txt
    psmf->Wzero11 *= 0.99746094; // from WOPTC4wisph.txt
    psmf->Wzero12 *= 0.99833984; // from WOPTC6wisph.txt
    psmf->Wzero13 *= 0.99941406; // from WOPTC8wisph.txt
    psmf->Wzero14 *= 0.99785156; // from WOPTWuC4wisph.txt
    psmf->Wzero15 *= 1.002922; // from WOPT32GRDwisphNEW.txt
    psmf->Wzero16 *= 0.992400; // from WOPT32GRDISPHwisphNEW.txt
    psmf->Wzero17 *= 0.999687; // from WOPT64GRDwisphNEW.txt
    psmf->Wzero18 *= 1.001781; // from WOPT64GRDISPHwisphNEW.txt
    psmf->Wzero19 *= 0.999184; // from WOPT128GRDwisphNEW.txt
    psmf->Wzero20 *= 1.000212; // from WOPT128GRDISPHwisphNEW.txt
    psmf->Wzero21 *= 0.999300; // from WOPT256GRDwisphNEW.txt
    psmf->Wzero22 *= 1.000211; // from WOPT256GRDISPHwisphNEW.txt
}
else if(nSmooth <= 48){
    psmf->Wzero *= 0.9921875; // from WOPT24Dwisph.txt
    psmf->Wzero2 *= 0.99882812; // from WOPT32Dwisph.txt
    psmf->Wzero3 *= 0.99033203; // from WOPT64Dwisph.txt
    psmf->Wzero4 *= 0.98476562; // from WOPT128Dwisph.txt
    psmf->Wzero5 *= 0.98955078; // from WOPT256Dwisph.txt
    psmf->Wzero6 *= 0.80622559; // from WOPTCM04wisph.txt
    psmf->Wzero7 *= 0.95644531; // from WOPTWuC2wisph.txt
    psmf->Wzero8 *= 0.99804688; // from WOPTBUHwisph.txt
    psmf->Wzero9 *= 1.00097656; // from WOPTCM05wisph.txt
    psmf->Wzero10 *= 0.99355469; // from WOPTC2wisph.txt
    psmf->Wzero11 *= 0.99931641; // from WOPTC4wisph.txt
    psmf->Wzero12 *= 0.99960938; // from WOPTC6wisph.txt
    psmf->Wzero13 *= 0.99941406; // from WOPTC8wisph.txt
    psmf->Wzero14 *= 0.996875; // from WOPTWuC4wisph.txt
    psmf->Wzero15 *= 1.000000; // from WOPT32GRDwisphNEW.txt
    psmf->Wzero16 *= 0.969901; // from WOPT32GRDISPHwisphNEW.txt
    psmf->Wzero17 *= 0.999380; // from WOPT64GRDwisphNEW.txt
    psmf->Wzero18 *= 1.000542; // from WOPT64GRDISPHwisphNEW.txt
    psmf->Wzero19 *= 0.998680; // from WOPT128GRDwisphNEW.txt
    psmf->Wzero20 *= 1.000209; // from WOPT128GRDISPHwisphNEW.txt
    psmf->Wzero21 *= 0.998906; // from WOPT256GRDwisphNEW.txt
    psmf->Wzero22 *= 1.000208; // from WOPT256GRDISPHwisphNEW.txt
}
else if(nSmooth <= 64){
    psmf->Wzero *= 0.99033203; // from WOPT24Dwisph.txt
    psmf->Wzero2 *= 0.99824219; // from WOPT32Dwisph.txt
    psmf->Wzero3 *= 0.97578125; // from WOPT64Dwisph.txt
    psmf->Wzero4 *= 0.97119141; // from WOPT128Dwisph.txt
    psmf->Wzero5 *= 0.98037109; // from WOPT256Dwisph.txt
    psmf->Wzero6 *= 0.71894531; // from WOPTCM04wisph.txt
    psmf->Wzero7 *= 0.93652344; // from WOPTWuC2wisph.txt
    psmf->Wzero8 *= 0.99394531; // from WOPTBUHwisph.txt
    psmf->Wzero9 *= 0.99755859; // from WOPTCM05wisph.txt
    psmf->Wzero10 *= 0.98828125; // from WOPTC2wisph.txt
    psmf->Wzero11 *= 0.99863281; // from WOPTC4wisph.txt
    psmf->Wzero12 *= 1.00009766; // from WOPTC6wisph.txt
    psmf->Wzero13 *= 1.00039063; // from WOPTC8wisph.txt
    psmf->Wzero14 *= 0.99658203; // from WOPTWuC4wisph.txt
    psmf->Wzero15 *= 0.999471; // from WOPT32GRDwisphNEW.txt
    psmf->Wzero16 *= 0.885784; // from WOPT32GRDISPHwisphNEW.txt
    psmf->Wzero17 *= 0.998768; // from WOPT64GRDwisphNEW.txt
    psmf->Wzero18 *= 1.000319; // from WOPT64GRDISPHwisphNEW.txt
    psmf->Wzero19 *= 0.998180; // from WOPT128GRDwisphNEW.txt
    psmf->Wzero20 *= 0.999691; // from WOPT128GRDISPHwisphNEW.txt
    psmf->Wzero21 *= 0.998315; // from WOPT256GRDwisphNEW.txt
    psmf->Wzero22 *= 0.999691; // from WOPT256GRDISPHwisphNEW.txt
}
else if(nSmooth <= 96){
    psmf->Wzero *= 0.98574219; // from WOPT24Dwisph.txt
    psmf->Wzero2 *= 0.99707031; // from WOPT32Dwisph.txt
    psmf->Wzero3 *= 0.95380859; // from WOPT64Dwisph.txt
    psmf->Wzero4 *= 0.93710937; // from WOPT128Dwisph.txt
    psmf->Wzero5 *= 0.98203125; // from WOPT256Dwisph.txt
    psmf->Wzero6 *= 0.55703125; // from WOPTCM04wisph.txt
    psmf->Wzero7 *= 0.8953125; // from WOPTWuC2wisph.txt
    psmf->Wzero8 *= 0.99882812; // from WOPTBUHwisph.txt
    psmf->Wzero9 *= 0.99882812; // from WOPTCM05wisph.txt
    psmf->Wzero10 *= 0.99804688; // from WOPTC2wisph.txt
    psmf->Wzero11 *= 0.99794922; // from WOPTC4wisph.txt
    psmf->Wzero12 *= 0.99990234; // from WOPTC6wisph.txt
    psmf->Wzero13 *= 0.99990234; // from WOPTC8wisph.txt
    psmf->Wzero14 *= 0.99277344; // from WOPTWuC4wisph.txt
    psmf->Wzero15 *= 0.999166; // from WOPT32GRDwisphNEW.txt
    psmf->Wzero16 *= 0.802548; // from WOPT32GRDISPHwisphNEW.txt
    psmf->Wzero17 *= 0.997967; // from WOPT64GRDwisphNEW.txt
    psmf->Wzero18 *= 0.999162; // from WOPT64GRDISPHwisphNEW.txt
    psmf->Wzero19 *= 0.996184; // from WOPT128GRDwisphNEW.txt
    psmf->Wzero20 *= 1.041391; // from WOPT128GRDISPHwisphNEW.txt
    psmf->Wzero21 *= 0.998225; // from WOPT256GRDwisphNEW.txt
    psmf->Wzero22 *= 0.999084; // from WOPT256GRDISPHwisphNEW.txt
}
else if(nSmooth <= 128){
    psmf->Wzero *= 0.98125; // from WOPT24Dwisph.txt
    psmf->Wzero2 *= 0.99433594; // from WOPT32Dwisph.txt
    psmf->Wzero3 *= 0.98037109; // from WOPT64Dwisph.txt
    psmf->Wzero4 *= 0.98085937; // from WOPT128Dwisph.txt
    psmf->Wzero5 *= 0.97641602; // from WOPT256Dwisph.txt
    psmf->Wzero6 *= 0.39453125; // from WOPTCM04wisph.txt
    psmf->Wzero7 *= 0.853125; // from WOPTWuC2wisph.txt
    psmf->Wzero8 *= 0.9984375; // from WOPTBUHwisph.txt
    psmf->Wzero9 *= 0.99736328; // from WOPTCM05wisph.txt
    psmf->Wzero10 *= 0.99765625; // from WOPTC2wisph.txt
    psmf->Wzero11 *= 0.99951172; // from WOPTC4wisph.txt
    psmf->Wzero12 *= 0.99970703; // from WOPTC6wisph.txt
    psmf->Wzero13 *= 1.0; // from WOPTC8wisph.txt
    psmf->Wzero14 *= 0.99433594; // from WOPTWuC4wisph.txt
    psmf->Wzero15 *= 0.996494; // from WOPT32GRDwisphNEW.txt
    psmf->Wzero16 *= 0.713914; // from WOPT32GRDISPHwisphNEW.txt
    psmf->Wzero17 *= 0.995968; // from WOPT64GRDwisphNEW.txt
    psmf->Wzero18 *= 0.995869; // from WOPT64GRDISPHwisphNEW.txt
    psmf->Wzero19 *= 0.994710; // from WOPT128GRDwisphNEW.txt
    psmf->Wzero20 *= 0.996480; // from WOPT128GRDISPHwisphNEW.txt
    psmf->Wzero21 *= 0.997739; // from WOPT256GRDwisphNEW.txt
    psmf->Wzero22 *= 0.996672; // from WOPT256GRDISPHwisphNEW.txt
}
else if(nSmooth <= 256){
    psmf->Wzero *= 0.98623047; // from WOPT24Dwisph.txt
    psmf->Wzero2 *= 0.99453125; // from WOPT32Dwisph.txt
    psmf->Wzero3 *= 0.97167969; // from WOPT64Dwisph.txt
    psmf->Wzero4 *= 0.97109375; // from WOPT128Dwisph.txt
    psmf->Wzero5 *= 0.97265625; // from WOPT256Dwisph.txt
    psmf->Wzero6 *= 0.1; // from WOPTCM04wisph.txt
    psmf->Wzero7 *= 0.69296875; // from WOPTWuC2wisph.txt
    psmf->Wzero8 *= 0.99824219; // from WOPTBUHwisph.txt
    psmf->Wzero9 *= 0.99814453; // from WOPTCM05wisph.txt
    psmf->Wzero10 *= 0.99599609; // from WOPTC2wisph.txt
    psmf->Wzero11 *= 0.99931641; // from WOPTC4wisph.txt
    psmf->Wzero12 *= 0.99980469; // from WOPTC6wisph.txt
    psmf->Wzero13 *= 0.99990234; // from WOPTC8wisph.txt
    psmf->Wzero14 *= 0.99492188; // from WOPTWuC4wisph.txt
    psmf->Wzero15 *= 0.996167; // from WOPT32GRDwisphNEW.txt
    psmf->Wzero16 *= 0.340680; // from WOPT32GRDISPHwisphNEW.txt
    psmf->Wzero17 *= 0.996920; // from WOPT64GRDwisphNEW.txt
    psmf->Wzero18 *= 0.997378; // from WOPT64GRDISPHwisphNEW.txt
    psmf->Wzero19 *= 0.996740; // from WOPT128GRDwisphNEW.txt
    psmf->Wzero20 *= 0.997920; // from WOPT128GRDISPHwisphNEW.txt
    psmf->Wzero21 *= 0.998239; // from WOPT256GRDwisphNEW.txt
    psmf->Wzero22 *= 0.997715; // from WOPT256GRDISPHwisphNEW.txt
}
else if(nSmooth <= 512){
    psmf->Wzero *= 0.98535156; // from WOPT24Dwisph.txt
    psmf->Wzero2 *= 0.99335938; // from WOPT32Dwisph.txt
    psmf->Wzero3 *= 0.95117187; // from WOPT64Dwisph.txt
    psmf->Wzero4 *= 0.94648437; // from WOPT128Dwisph.txt
    psmf->Wzero5 *= 0.98125; // from WOPT256Dwisph.txt
    psmf->Wzero6 *= 0.94960937; // from WOPTCM04wisph.txt
    psmf->Wzero7 *= 0.3625; // from WOPTWuC2wisph.txt
    psmf->Wzero8 *= 0.99804688; // from WOPTBUHwisph.txt
    psmf->Wzero9 *= 0.99707031; // from WOPTCM05wisph.txt
    psmf->Wzero10 *= 0.99658203; // from WOPTC2wisph.txt
    psmf->Wzero11 *= 0.99931641; // from WOPTC4wisph.txt
    psmf->Wzero12 *= 0.99980469; // from WOPTC6wisph.txt
    psmf->Wzero13 *= 0.99990234; // from WOPTC8wisph.txt
    psmf->Wzero14 *= 0.99316406; // from WOPTWuC4wisph.txt
    psmf->Wzero15 *= 0.994534; // from WOPT32GRDwisphNEW.txt
    psmf->Wzero16 *= 0.087656; // from WOPT32GRDISPHwisphNEW.txt
    psmf->Wzero17 *= 0.995466; // from WOPT64GRDwisphNEW.txt
    psmf->Wzero18 *= 0.996125; // from WOPT64GRDISPHwisphNEW.txt
    psmf->Wzero19 *= 0.995776; // from WOPT128GRDwisphNEW.txt
    psmf->Wzero20 *= 0.996361; // from WOPT128GRDISPHwisphNEW.txt
    psmf->Wzero21 *= 0.998143; // from WOPT256GRDwisphNEW.txt
    psmf->Wzero22 *= 0.996157; // from WOPT256GRDISPHwisphNEW.txt
}
 #endif

 psmf->Wzero *= msr->param.dnloc;
 psmf->Wzero2 *= msr->param.dnloc;
 psmf->Wzero3 *= msr->param.dnloc;
 psmf->Wzero4 *= msr->param.dnloc;
 psmf->Wzero5 *= msr->param.dnloc;
 psmf->Wzero6 *= msr->param.dnloc;
 psmf->Wzero7 *= msr->param.dnloc;
 psmf->Wzero8 *= msr->param.dnloc;
 psmf->Wzero9 *= msr->param.dnloc;
 psmf->Wzero10 *= msr->param.dnloc;
 psmf->Wzero11 *= msr->param.dnloc;
 psmf->Wzero12 *= msr->param.dnloc;
 psmf->Wzero13 *= msr->param.dnloc;
 psmf->Wzero14 *= msr->param.dnloc;
 psmf->Wzero15 *= msr->param.dnloc;
 psmf->Wzero16 *= msr->param.dnloc;
 psmf->Wzero17 *= msr->param.dnloc;
 psmf->Wzero18 *= msr->param.dnloc;
 psmf->Wzero19 *= msr->param.dnloc;
				...
				int *pnActive)
{
    ...
    inExt.bGalaxyDiskVerticalPotential = 0;
#ifdef ROT_FRAME
	inExt.bRotFrame = 0;
#endif
#ifdef SLIDING_PATCH
	...
	if (msr->param.bPatch || msr->param.bSimpleGasDrag) {
	  ...
	  if (msr->param.bPatch) {
		...
		if (bFirstCall) {
          ...
          if (!msr->param.bPatchVerticalGravity) PP->dOrbFreqZ2 = 0;
		  if (msr->param.bVStart)
              (void) printf("Patch: vert. freq. enhancement factor = %g\n",
						  sqrt(PP->dOrbFreqZ2)/PP->dOrbFreq);
          printf("Sliding patch: Radial frequency %f  Vertical Frequency %f\n",PP->dOrbFreq,sqrt(PP->dOrbFreqZ2));
		  bFirstCall = 0;
		}
	...
	*pdEMax = out.dEMax;
	...
	void msrCalcEandL(MSR msr,int bFirst,double dTime,double *E,double *T,
			  double *U,double *Eth,double L[],double cm[],double Llin[],double MW[],double RS[],double *Emag,
			  double *S, double *Se, double *divBAvg, double *divBMax, double *divBerrAvg, double *divBerrMax,
			  double *alphaAvg, double *alphaMax, double *betaMax, double *betaAvg, double *betaMin, double *etaresAvg, double *kinviscAvg,
			  double *rmsmach, double *vrms, double *rhogasAvg, double *rhogasMax,
			  double *Q1Avg, double *Q1Max, double *Q2Avg, double *Q2Max, double *E0Avg, double *E0Max, double *Q4Avg, double *Q4Max)
	{
		...
		*Se = out.Se;
		...
		for (k=0;k<3;k++){
		  L[k] = out.L[k];
		  cm[k] = out.cm[k]/(out.dMass);
		  Llin[k]=out.Llin[k];
			MW[k]=out.MW[k];
			RS[k]=out.RS[k];
		}
		*vrms= sqrt(out.vrms/n);
		*rmsmach= sqrt(out.rmsmach/ngas);
		*divBerrAvg= out.divBerrAvg/ngas;
		*divBAvg= out.divBAvg/ngas;
		*alphaAvg= out.alphaAvg/ngas;
		if(out.B2sum>0.0){*betaAvg= out.betaAvg/out.B2sum;}
		*etaresAvg= out.etaresAvg/ngas;
                printf(" *ICr0 %g ",*etaresAvg);
		*kinviscAvg= out.kinviscAvg/ngas;
		*rhogasAvg= out.rhogasAvg/ngas;
		*Q1Avg= out.Q1Avg/ngas;
		*Q2Avg= out.Q2Avg/ngas;
		*E0Avg= out.E0Avg/ngas;
		*Q4Avg= out.Q4Avg/ngas;
		*Q1Max = out.Q1Max;
		*Q2Max = out.Q2Max;
		*E0Max = out.E0Max;
		*Q4Max = out.Q4Max;
		*divBerrMax = out.divBerrMax;
		*divBMax = out.divBMax;
		*betaMin = out.betaMin;
		*betaMax = out.betaMax;
		*alphaMax = out.alphaMax;
		*rhogasMax = out.rhogasMax;
		...
		*U *= a;
		...
		if (bFirst)
			...
			*T -= msr->dTcoll;
	#endif
		*E = (*T) + (*U) - msr->dEcosmo + a*a*(*Eth) + a*a*(*Emag);
		}
...
void msrDrift(MSR msr,double dTime,double dDelta)
{
	...
	if (msr->param.bCannonical) {
		...
		printf("%f %f %f ADSSDSASDSAADS",invpr.dvFacOne,invpr.dvFacTwo,dDelta);
...
#ifdef SLIDING_PATCH	/* Gas patch */
	invpr.dOrbFreq = msr->param.PP.dOrbFreq;
#endif
	if (dDelta != 0.0) {
		struct outKick out;
		int nout;
...
#ifdef SLIDING_PATCH
	    in.bOpen = 1;
	    in.dOrbFreq = msr->param.PP.dOrbFreq;
#endif
	    pstKick(msr->pst,&in,sizeof(in),&out,NULL);
	    }
...
#endif
#ifdef BAROTROPIC
	// add input parameters for eq
	in.gpc.rho0= (5.0279933*1e18/msr->param.dMsolUnit)/(pow(3.24077929/msr->param.dKpcUnit,3));  // msolpc^-3
	in.gpc.cs0=((0.2*1e-17*3.24077929/msr->param.dKpcUnit)*msr->param.dSecUnit); //(m/s)
#endif
		/*
...
void msrICGetFmax(MSR msr,double *fmax)
{
  struct outICGetFmax out;
  int ngas = msr->nGas;
  double fmaxinit = 0.0;
  ...
  printf("old Q1 %g icfmax %f ICR0 %g", msr->param.dICQ1Avg,msr->param.dICFmax,msr->param.dICR0);
  pstICGetFmax(msr->pst,&fmaxinit,sizeof(double),&out,NULL);
  out.ICvelAvg = sqrt(out.ICvelAvg)/ngas;
  out.hAvg=out.hAvg/ngas;
  *fmax=out.fmax;
  printf("old Q1 %g new Q1 %g Q1diff %g icfmax %f ICR0 %g havg %g", msr->param.dICQ1Avg,out.ICvelAvg,msr->param.dICQ1Avg-out.ICvelAvg,*fmax,msr->param.dICR0,out.hAvg);

  if(msr->bStepZero == 1){
    msr->param.dICR0=msr->param.dICR0/out.hAvg;
    msr->param.dICRLOOP0 = msr->param.dICR0*msr->param.dICRLOOP0;
    printf("Setting ICR0 so that particles move far through domain even at higher resolution.");
  }
  //decay speed will be set by difference, if move resulted in higher velocities(density difference) reduce speed
  // if move resulted in lower, increase velocity again but by smaller factor.
  // idea is that, when we find a minimum, we will mainly have bad moves and velocity reduce to zero.
  // To not get stuck in local minimum maybe need to increase if several bad moves ?
  // Starting with large initiial seem to work quite well
  if(msr->param.dICQ1Avg-out.ICvelAvg < 0.0){
    msr->param.dICR0 = msr->param.dICR0/rate;
  }
  else{
    msr->param.dICR0 = msr->param.dICR0*rateup;
  }
  msr->param.dICQ1Avg = out.ICvelAvg;
}

void msrICNormalizeFmax(MSR msr,double fmax)
 {
   struct outICNormalizeFmax out;
   pstICNormalizeFmax(msr->pst,&fmax,sizeof(double),&out,NULL);
 }

void msrICNormalizeVel(MSR msr)
 {
   double fmax;
   pstICNormalizeVel(msr->pst,&fmax,sizeof(double),NULL,NULL);
 }
...
#endif
				#ifdef ISPH
				msrReSmooth(msr,dTime,SMX_ISPHGRADTERMS,1);
...
msrReSmooth(msr,dTime,SMX_ICGENERATOR,1);
 printf("\n Getting fmax\n");
 double fmax = 0.0;
 msrICGetFmax(msr,&fmax);
  printf("\n Normalizing fmax\n");
 msrICNormalizeFmax(msr,fmax);
 printf("\n Normalizing velocities\n");
msrICNormalizeVel(msr);
#endif
msrBallMax(msr,iKickRung,1);
...
#ifdef SETPRANDTL
    double PmFac=1.0;
    msrCalcPmFactor(msr, 1.6, &PmFac);
    msrUpdateBDot(msr,PmFac);
    msrCalcPmFactor(msr, 1.6, &PmFac);
#endif
#else
 ...
 void msrCalcPmFactor(MSR msr,double PmVal,double *PmFac)
   {
     struct outCalcPmFactor out;
     double Pm;
     pstCalcPmFactor(msr->pst,NULL,0,&out,NULL);
     Pm = (out.etaresAvg > 0 ? out.kinviscAvg/out.etaresAvg : PmVal);
#ifdef SETPRANDTLVISC
     *PmFac= (Pm > PmVal/10.0 ? PmVal/Pm : 10.0 );
     if(*PmFac<1.0){*PmFac=1.0;}
#else
     *PmFac=Pm/PmVal;
     if(*PmFac>1.0){*PmFac=1.0;}
#endif
     printf("kin %f eta %f Pm %f",out.kinviscAvg,out.etaresAvg,Pm);
 }
 void msrUpdateBDot(MSR msr,double PmFac)
 {
   pstUpdateBDot(msr->pst,&PmFac,sizeof(double),NULL,NULL);
 }

 void msrISPHInvertMatrix(MSR msr)
 {
