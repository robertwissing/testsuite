...
pstAddServices(PST pst,MDL mdl)
{
	...
	mdlAddService(mdl,PST_GETGASPRESSURE,pst,
        ...
        mdlAddService(mdl,PST_CALCPMFACTOR,pst,
		      (void (*)(void *,void *,int,void *,int *)) pstCalcPmFactor,
		      0,sizeof(struct outCalcPmFactor));
        mdlAddService(mdl,PST_UPDATEBDOT,pst,
		      (void (*)(void *,void *,int,void *,int *)) pstUpdateBDot,
		      sizeof(double),0);
      	mdlAddService(mdl,PST_ISPHINVERTMATRIX,pst,
		      (void (*)(void *,void *,int,void *,int *)) pstISPHInvertMatrix,
		      0,0);
	mdlAddService(mdl,PST_UPDATEDENSITY,pst,
        ...
        mdlAddService(mdl,PST_ICGETFMAX,pst,
	        ...
	        mdlAddService(mdl,PST_ICNORMALIZEFMAX,pst,
                      (void (*)(void *,void *,int,void *,int *)) pstICNormalizeFmax,
                      sizeof(double),sizeof(struct outICNormalizeFmax));
        mdlAddService(mdl,PST_ICNORMALIZEVEL,pst,
                      (void (*)(void *,void *,int,void *,int *)) pstICNormalizeVel,
                      sizeof(double),0);
...
void pstPostVariableSoft(PST pst,void *vin,int nIn,void *vout,int *pnOut)
{
	...
	if (pst->nLeaves > 1) {
	  ...
	  pkdPostVariableSoft(plcl->pkd,in->dSoftMax,in->bSoftMaxMul,in->iVariableSoftType,in->nSmoothMean);
		}
	if (pnOut) *pnOut = 0;
	}
#endif
...
void pstCalcEandL(PST pst,void *vin,int nIn,void *vout,int *pnOut)
{
	...
	if (pst->nLeaves > 1) {
		...
		out->U += outLcl.U;
		out->Eth += outLcl.Eth;
		out->Emag += outLcl.Emag;
		out->S += outLcl.S;
		out->Se += outLcl.Se;
		out->dMass += outLcl.dMass;
		out->divBAvg += outLcl.divBAvg;
		out->divBerrAvg += outLcl.divBerrAvg;
		out->alphaAvg += outLcl.alphaAvg;
		out->betaAvg += outLcl.betaAvg;
		out->etaresAvg += outLcl.etaresAvg;
		out->kinviscAvg += outLcl.kinviscAvg;
		out->Q1Avg += outLcl.Q1Avg;
		out->Q2Avg += outLcl.Q2Avg;
		out->E0Avg += outLcl.E0Avg;
		out->Q4Avg += outLcl.Q4Avg;
		out->Q1Max = max(out->Q1Max,outLcl.Q1Max);
		out->Q2Max = max(out->Q2Max,outLcl.Q2Max);
		out->E0Max = max(out->E0Max,outLcl.E0Max);
		out->Q4Max = max(out->Q4Max,outLcl.Q4Max);
		out->rmsmach += outLcl.rmsmach;
		out->vrms += outLcl.vrms;
		out->betaMax = max(out->betaMax,outLcl.betaMax);
		out->alphaMax = max(out->alphaMax,outLcl.alphaMax);
		out->betaMin = min(out->betaMin,outLcl.betaMin);
		out->divBMax = max(out->divBMax,outLcl.divBMax);
		out->divBerrMax = max(out->divBerrMax,outLcl.divBerrMax);
		out->rhogasAvg += outLcl.rhogasAvg;
                out->rhogasMax = max(out->rhogasMax,outLcl.rhogasMax);
                out->B2sum += outLcl.B2sum;
		for (k=0;k<3;k++) {
		  out->L[k] += outLcl.L[k];
		  out->cm[k] += outLcl.cm[k];
		  out->Llin[k] += outLcl.Llin[k];
		  out->MW[k] += outLcl.MW[k];
			out->RS[k] += outLcl.RS[k];
		}
		}
	else {
		pkdCalcEandL(plcl->pkd,&out->T,&out->U,&out->Eth,out->L,out->cm,out->Llin,out->MW,out->RS,&out->Emag,&out->S,
			     &out->Se,&out->dMass,&out->divBAvg, &out->divBMax, &out->divBerrAvg, &out->divBerrMax,
			     &out->alphaAvg,&out->alphaMax, &out->betaMax, &out->betaAvg, &out->betaMin, &out->etaresAvg,
			     &out->kinviscAvg,&out->rmsmach,&out->vrms,&out->rhogasAvg,&out->rhogasMax,&out->B2sum
			     ,&out->Q1Avg,&out->Q1Max,&out->Q2Avg,&out->Q2Max,&out->E0Avg,&out->E0Max,&out->Q4Avg,&out->Q4Max);
		}
...
void pstGetGasPressure(PST pst,void *vin,int nIn,void *vout,int *pnOut)
{
 ...
 void pstCalcPmFactor(PST pst,void *vin,int nIn,void *vout,int *pnOut)
 {
   LCL *plcl = pst->plcl;
 struct outCalcPmFactor *out = vout;
 struct outCalcPmFactor outLcl;
 int k;

 mdlassert(pst->mdl,nIn == 0);
 if (pst->nLeaves > 1) {
   mdlReqService(pst->mdl,pst->idUpper,PST_CALCPMFACTOR,NULL,0);
   pstCalcPmFactor(pst->pstLower,NULL,0,out,NULL);
   mdlGetReply(pst->mdl,pst->idUpper,&outLcl,NULL);
   out->etaresAvg += outLcl.etaresAvg;
   out->kinviscAvg += outLcl.kinviscAvg;
 }
 else {
   pkdCalcPmFactor(plcl->pkd,&out->etaresAvg,&out->kinviscAvg);
 }
 if (pnOut) *pnOut = sizeof(struct outCalcPmFactor);
        }

 void pstUpdateBDot(PST pst,void *vin,int nIn,void *vout,int *pnOut)
 {
   LCL *plcl = pst->plcl;
   double *PmFac = vin;

   if (pst->nLeaves > 1) {
     mdlReqService(pst->mdl,pst->idUpper,PST_UPDATEBDOT,vin,nIn);
     pstUpdateBDot(pst->pstLower,vin,nIn,NULL,NULL);
     mdlGetReply(pst->mdl,pst->idUpper,NULL,NULL);
   }
   else {
     pkdUpdateBDot(plcl->pkd, *PmFac);
   }
   if (pnOut) *pnOut = 0;
 }

 void pstISPHInvertMatrix(PST pst,void *vin,int nIn,void *vout,int *pnOut)
 {
   LCL *plcl = pst->plcl;
   if (pst->nLeaves > 1) {
     mdlReqService(pst->mdl,pst->idUpper,PST_ISPHINVERTMATRIX,vin,nIn);
     pstISPHInvertMatrix(pst->pstLower,vin,nIn,NULL,NULL);
     mdlGetReply(pst->mdl,pst->idUpper,NULL,NULL);
   }
   else {
     pkdISPHInvertMatrix(plcl->pkd);
   }
   if (pnOut) *pnOut = 0;
 }
...
void pstGetDensityU(PST pst,void *vin,int nIn,void *vout,int *pnOut)
{
 ...
 void pstICGetFmax(PST pst,void *vin,int nIn,void *vout,int *pnOut)
 {
   LCL *plcl = pst->plcl;
   struct outICGetFmax *out = vout;
   struct outICGetFmax outLcl;
   double *infmax = vin;
   if (pst->nLeaves > 1) {
     mdlReqService(pst->mdl,pst->idUpper,PST_ICGETFMAX,vin,nIn);
     pstICGetFmax(pst->pstLower,vin,nIn,out,NULL);
     mdlGetReply(pst->mdl,pst->idUpper,&outLcl,NULL);
     out->ICvelAvg += outLcl.ICvelAvg;
     out->fmax = max(out->fmax,outLcl.fmax);
     out->hAvg += outLcl.hAvg;
   }
   else {
     pkdICGetFmax(plcl->pkd,infmax,&out->fmax,&out->ICvelAvg,&out->hAvg);
   }
   if (pnOut) *pnOut = sizeof(struct outICGetFmax);
 }

 void pstICNormalizeFmax(PST pst,void *vin,int nIn,void *vout,int *pnOut)
 {
   LCL *plcl = pst->plcl;
   double *infmax = vin;
   struct outICNormalizeFmax *out = vout;
   struct outICNormalizeFmax outLcl;
   if (pst->nLeaves > 1) {
     mdlReqService(pst->mdl,pst->idUpper,PST_ICNORMALIZEFMAX,vin,nIn);
     pstICNormalizeFmax(pst->pstLower,vin,nIn,out,NULL);
     mdlGetReply(pst->mdl,pst->idUpper,&outLcl,NULL);
     out->fmaxnorm += outLcl.fmaxnorm;
   }
   else {
     pkdICNormalizeFmax(plcl->pkd,infmax,&out->fmaxnorm);
   }
   if (pnOut) *pnOut = 0;
 }

 void pstICNormalizeVel(PST pst,void *vin,int nIn,void *vout,int *pnOut)
 {
   LCL *plcl = pst->plcl;
   double *infmax = vin;
   if (pst->nLeaves > 1) {
     mdlReqService(pst->mdl,pst->idUpper,PST_ICNORMALIZEVEL,vin,nIn);
     pstICNormalizeVel(pst->pstLower,vin,nIn,NULL,NULL);
     mdlGetReply(pst->mdl,pst->idUpper,NULL,NULL);
   }
   else {
     pkdICNormalizeVel(plcl->pkd,infmax);
   }
   if (pnOut) *pnOut = 0;
 }
...
void pstKickVpred(PST pst,void *vin,int nIn,void *vout,int *pnOut)
{
	...
	if (pst->nLeaves > 1) {
		...
		pkdKickVpred(plcl->pkd,in->dvFacOne,in->dvFacTwo,in->duDelta,
		    in->iGasModel,in->z,in->duDotLimit,in->dTimeEnd,in->uhc,in->dOrbFreq);
		out->Time = pkdGetTimer(plcl->pkd,1);
		out->MaxTime = out->Time;
		out->SumTime = out->Time;
		out->nSum = 1;
