...
#endif
{
	...
	double dTime;
	double E=0,T=0,U=0,Eth=0,L[3]={0,0,0},cm[3]={0,0,0},Llin[3]={0,0,0},MW[3]={0,0,0},RS[3]={0,0,0},Emag=0,S=0,Se=0,divBAvg=0,divBMax=0,divBerrAvg=0,divBerrMax=0,alphaAvg=0,alphaMax=0,betaMax=0,betaAvg=0,betaMin=0,rmsmach=0,vrms=0,rhogasAvg=0,rhogasMax=0,Q1Avg=0.0,Q1Max=0.0,Q2Avg=0.0,Q2Max=0.0,E0Avg=0.0,E0Max=0.0,Q4Avg=0.0,Q4Max=0.0;
	double kinviscAvg=0,etaresAvg=0,dWMax=0,dIMax=0,dEMax=0,dMass=0,dMultiEff=0;
	long lSec=0,lStart;
...
*/
 ...
 CheckForDiagnosticOutput:
	if (msrSteps(msr) > 0) {
		...
		msrInitSph(msr,dTime);
#endif
msrCalcEandL(msr,MSR_INIT_E,dTime,&E,&T,&U,&Eth,L,cm,Llin,MW,RS,&Emag,&S,&Se,&divBAvg,&divBMax,
			 &divBerrAvg, &divBerrMax, &alphaAvg, &alphaMax, &betaMax, &betaAvg, &betaMin, &etaresAvg,&kinviscAvg,
	     &rmsmach, &vrms,&rhogasAvg,&rhogasMax,&Q1Avg,&Q1Max,&Q2Avg,&Q2Max,&E0Avg,&E0Max,&Q4Avg,&Q4Max);
msrMassCheck(msr,dMass,"After msrCalcEandL");
dMultiEff = 1.0;
if (msrLogInterval(msr)) {
	(void) fprintf(fpLog,"%.4e %.4e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e"
		 "%i %.4e %.4e %.4e %.4e\n",dTime,
		 1.0/csmTime2Exp(msr->param.csm,dTime)-1.0,
		 E,T,U,Eth,Emag,S,Se,L[0],L[1],L[2],Llin[0],Llin[1],Llin[2],cm[0],cm[1],
		 cm[2],MW[0],MW[1],MW[2],RS[0],RS[1],RS[2], divBAvg,divBMax, divBerrAvg, divBerrMax, alphaAvg, alphaMax,
		 betaMax,betaAvg, betaMin, etaresAvg, kinviscAvg, rmsmach, vrms,rhogasAvg,rhogasMax,Q1Avg,Q1Max,
		       Q2Avg,Q2Max,E0Avg,E0Max,Q4Avg,Q4Max,iSec,dWMax,dIMax,dEMax,dMultiEff);
}
#ifdef INFLOWOUTFLOW
		if (msr->param.bInflowOutflow) msrModifyAccel(msr,dTime); /* zero acceleration of inflow/outflow particles */
#endif
			...
			if (msrKDK(msr)) {
				...
				*/
				if (msrLogInterval(msr) && iStep%msrLogInterval(msr) == 0) {
				  msrCalcEandL(msr,MSR_INIT_E,dTime,&E,&T,&U,&Eth,L,cm,Llin,MW,RS,&Emag,&S,&Se,&divBAvg,
					       &divBMax, &divBerrAvg, &divBerrMax, &alphaAvg, &alphaMax, &betaMax,
					       &betaAvg, &betaMin, &etaresAvg, &kinviscAvg, &rmsmach, &vrms,&rhogasAvg,&rhogasMax,&Q1Avg,&Q1Max,&Q2Avg,&Q2Max,&E0Avg,&E0Max,&Q4Avg,&Q4Max);
				  msrMassCheck(msr,dMass,"After msrCalcEandL in KDK");
				  (void) fprintf(fpLog,"%.4e %.4e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e %.12e "
						 "%i %.4e %.4e %.4e %.4e\n",dTime,
						 1.0/csmTime2Exp(msr->param.csm,dTime)-1.0,
						 E,T,U,Eth,Emag,S,Se,L[0],L[1],L[2],Llin[0],Llin[1],Llin[2],cm[0],cm[1],cm[2],MW[0],MW[1],MW[2],RS[0],RS[1],RS[2],divBAvg,divBMax, divBerrAvg, divBerrMax, alphaAvg, alphaMax, betaMax,betaAvg, betaMin, etaresAvg, kinviscAvg, rmsmach, vrms,rhogasAvg,rhogasMax,Q1Avg,Q1Max,Q2Avg,Q2Max,E0Avg,E0Max,Q4Avg,Q4Max,iSec,dWMax,dIMax,dEMax,
						 dMultiEff);
				}
				LogTimingOutput( msr, fpLogTiming, dTime, 0 );
			}
				...
				if (msrLogInterval(msr) && iStep%msrLogInterval(msr) == 0) {
					...
					msrCalcEandL(msr,MSR_INIT_E,dTime,&E,&T,&U,&Eth,L,cm,Llin,MW,RS,&Emag,&S,&Se,
						     &divBAvg,&divBMax,&divBerrAvg, &divBerrMax, &alphaAvg,
						     &alphaMax, &betaMax, &betaAvg, &betaMin,&etaresAvg,&kinviscAvg,&rmsmach,
						     &vrms,&rhogasAvg,&rhogasMax,&Q1Avg,&Q1Max,&Q2Avg,&Q2Max,&E0Avg,&E0Max,&Q4Avg,&Q4Max);
