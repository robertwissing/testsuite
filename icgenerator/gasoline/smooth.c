...
int smInitialize(SMX *psmx,PKD pkd,SMF *smf,int nSmooth,int bPeriodic,
    ...
    case SMX_DENDVDX:
        ...
        break;
    case SMX_ICGENERATOR:
        smx->fcnSmooth = ICGenerator;
        initParticle = initICGeneratorParticle;
        initTreeParticle = NULL;
        init = initICGenerator;
        comb = combICGenerator;
        smx->fcnPost = NULL;
        break;
    case SMX_ISPHGRADTERMS:
        smx->fcnSmooth = ISphGradTerms;
        initParticle = initISphGradTermsParticle; /* Original Particle */
        initTreeParticle = NULL;
        init = initISphGradTerms; /* Cached copies */
        comb = combISphGradTerms;
        smx->fcnPost = NULL;
        break;
    case SMX_PRECORRDENS:
    smx->fcnSmooth = PreCorrDensPartition;
    initParticle = initPreCorrDensPartitionParticle; /* Original Particle */
    initTreeParticle = NULL;
    init = initPreCorrDensPartition; /* Cached copies */
    comb = combPreCorrDensPartition;
    smx->fcnPost = NULL;
    break;
    case SMX_CORRDENS:
    smx->fcnSmooth = CorrDensPartition;
    initParticle = initCorrDensPartitionParticle; /* Original Particle */
    initTreeParticle = NULL;
    init = initCorrDensPartition; /* Cached copies */
    comb = combCorrDensPartition;
    smx->fcnPost = NULL;
    break;
    case SMX_CORRENTROPY:
      smx->fcnSmooth = CorrEntropy;
      initParticle = initCorrEntropyParticle; /* Original Particle */
      initTreeParticle = NULL;
      init = initCorrEntropy; /* Cached copies */
      comb = combCorrEntropy;
      smx->fcnPost = NULL;
      break;
...
void smLargefBallCheck(SMX smx,PARTICLE *p,FLOAT lx, FLOAT ly, FLOAT lz) {
    ...
    */
    ...
    if ((lx < FLOAT_MAXVAL && p->fBall2 >= 0.249*lx*lx) ||
        (ly < FLOAT_MAXVAL && p->fBall2 >= 0.249*ly*ly) ||
        (lz < FLOAT_MAXVAL && p->fBall2 >= 0.249*lz*lz)) {
...
#ifdef SETMAXH3
	p->fBall2 = 0.249*lx*lx;
#endif
        }
    }

void smSetSmoothLength(SMX smx,PARTICLE *p) {
#ifdef ICGENold
// Smoothing length is set following density profile or set uniform density
// ph = sqrt(0.25*ball2)
// ball2 = 4*ph*ph
int nSmooth = smx->nSmooth;
 double ph, rad, V0, hfact3,eta,rho0,rpart;
int Rkern = 2.0;
 rpart = sqrt(p->r[0]*p->r[0]);
//V0 = p->fMass/smx->ICDensity or save addditional particle array

 //if(sqrt(p->r[0]*p->r[0]) > 0.25){rho0=0.5;}
 //else{rho0=1.5;}
 //rho0=(1.5-rpart*2.0);
 rho0=1.0;
//hfact3 = 3.0*smx->nSmooth*M_1_PI/(Rkern*Rkern*Rkern*4.0);
//eta = (1.7-0.9*pow(10,-smx->dTime/500.0));
eta=1.7;
hfact3 = eta*eta*eta;
// DIEHL 2015
//ph = pow(smx->nSmooth,1.0/3.0)*rad/2;
// ROSSWOG 2015
V0 = p->fMass/rho0;
ph = pow(hfact3*V0,1.0/3.0);
p->fBall2 = 4.0*ph*ph;
#endif
        }
    ...
    */
    ...
    while (cell != ROOT) {
        ...
        while (1) {
            ...
            bCellInt=0;
            if (bDebugCell) {
                double rp[4][3]={{-0.49218749468752376, -0.2131624523565443, -0.48717949208299349},
                    {-0.49218749468752376, -0.2131624523565443, 0.51282050791700651},
                    {0.50781250531247624,  0.32633754764345574, -0.48717949208299349},
                    {0.50781250531247624,  0.32633754764345574, 0.51282050791700651}};
                int ip,ii,jj;
                double dr2min=1e37;
                int ipmin=4;
                for (ip=0;ip<4;ip++) {
                    double dr2 = 0;
                    for (ii=0;ii<3;ii++) {
                        double drL = c[cp].bnd.fMin[ii]-rp[ip][ii];
                        double drR = rp[ip][ii]-c[cp].bnd.fMax[ii];
                        if (drL>0.0) dr2+=drL*drL;
                        else if (drR>0) dr2+=drR*drR;
                        }
                    if (dr2<dr2min) { dr2min=dr2; ipmin=ip; }
                    }
                if (dr2min<fBall2+1e-6) { bCellInt=1; printf("Intersect: %d  %d %f    %f %f %f  %f %f %f\n",cp,ipmin,dr2min,c[cp].bnd.fMin[0],c[cp].bnd.fMin[1],c[cp].bnd.fMin[2],c[cp].bnd.fMax[0],c[cp].bnd.fMax[1],c[cp].bnd.fMax[2]); }
                }
#endif
            INTERSECT(&c[cp],fBall2,lx,ly,lz,x,y,z,sx,sy,sz,bPeriodic,
                GetNext_1);
#ifdef SHEARTEST2
            if (bDebugCell) printf("INTERSECT: %d  %d %f    %f %f %f  %f %f %f\n",cp,c[cp].iDim,0.0,c[cp].bnd.fMin[0],c[cp].bnd.fMin[1],c[cp].bnd.fMin[2],c[cp].bnd.fMax[0],c[cp].bnd.fMax[1],c[cp].bnd.fMax[2]);
#endif
            if (c[cp].iDim >= 0) {
                cp = LOWER(cp);
                continue;
                ...
                for (pj=c[cp].pLower;pj<=pUpper;++pj) {
                    ...
                    if (bDebugCell) {
                        double rp[4][3]={
{-0.49218749468752376, -0.2131624523565443, -0.48717949208299349},
    {-0.49218749468752376, -0.2131624523565443, 0.51282050791700651},
        {0.50781250531247624,  0.32633754764345574, -0.48717949208299349},
            {0.50781250531247624,  0.32633754764345574, 0.51282050791700651}};
                        int ip,ii;
                        ...
                        for (ip=0;ip<4;ip++) {
                            double dr2 = 0;
                            for (ii=0;ii<3;ii++) {
                                double dr = p[pj].r[ii]-rp[ip][ii];
                                dr2+=dr*dr;
                                }
                            if (dr2<dr2min) { dr2min=dr2; ipmin=ip; }
                            }
                        if (dr2min<fBall2) { bPartInt=1; printf("Particle: %d  %d %f    %f %f %f\n",p[pj].iOrder,ipmin,dr2min,p[pj].r[0],p[pj].r[1],p[pj].r[2]); }
                        }
#endif
                    dx = sx - p[pj].r[0];
                    dy = sy - p[pj].r[1];
                    ...
                    fDist2 = dx*dx + dy*dy + dz*dz;
#ifdef SHEARTEST200
                    if (bDebugCell && fDist2 > fBall2) assert(!bPartInt);
#endif
                    if (fDist2 < fBall2) {
                        *cpStart = PARENT(cell);
#ifdef SHEARTEST200
                        if (bDebugCell) printf("PARTICLE: %d      %f %f %f\n",p[pj].iOrder,p[pj].r[0],p[pj].r[1],p[pj].r[2]);
#endif
...
#ifdef SHEARTEST2
                    //if (bDebugCell) assert(bCellInt);
            bCellInt=0;
#endif
        GetNext_1:
#ifdef SHEARTEST2
            if (bDebugCell) {
                    if (bCellInt) printf("NOT INTERSECTED!!!\n");
                    //assert(!bCellInt);
                    }
#endif
            SETNEXT(cp);
    ...
    for (pNext=0;pNext<nLocal;++pNext) {
        ...
        for (;;) { /* Snake Loop */
            ...
            */
            ...
            bDebugCell=0;
            if (p[pi].iOrder==64) {
                printf("smBallSearch for %d, fBall2 = %g\n",p[pi].iOrder,pq->fKey);
                if (smx->dTime > 0.3069) bDebugCell=1;
                }
#endif
            pq = smBallSearchAll(smx,pq,pi,x,y,z,lx,ly,lz);
#ifdef SHEARTEST2
            if (p[pi].iOrder==64) printf("smBallSearch AFTER %d, fBall2 = %g\n",p[pi].iOrder,pq->fKey);
#endif
            /*
            ...
            */
#ifdef SHEARTEST2000
            printf("Continue SNAKE: %d to %d (%d)\n",p[pi].iOrder,p[pqNext->p].iOrder,pi);
#endif
            pi = pqNext->p;
            x = p[pi].r[0];    y = p[pi].r[1];    z = p[pi].r[2];
