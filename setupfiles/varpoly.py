    ...
    def calculate_grun_cold(self,rho):
        ...
        for i in range(len(self.rho)):
            if(self.rho[i] >= self.rho0):
                ...
                self.grun_lamb[i]=((-self.B_c[i])*(1/self.grun[i]**2)*(self.B_c[i]*dgamma**2-self.grun[i]*(self.dBdP_c[i]*dgamma+self.B_c[i]*d2gamma)))
...
## Get isentrope from gruneisen    
    ...
    def calculate_grun_integral2(self,rho):
        self.checkmat(rho)
        self.calculate_grun_cold(rho)
    ...
    def calculate_grun_integral(self, rho):
        ...
        # Initialize grun_int with the correct shape
        self.grun_int = np.zeros_like(self.rho)
    
        # Precompute the integrand values over the entire range of rho
        grun_integrand_values = np.zeros_like(self.rho)
    
        for i in range(len(self.rho)):
            # Calculate the integrand from the start to the current index
        ...
        # Alternatively, if you need to use the quadrature method for some reason
        # Replace np.trapz with quad if necessary, and ensure the results are handled correctly.
