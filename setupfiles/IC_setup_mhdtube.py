...
class setup_mhdtube(object):
    ...
    def __init__(self):
        pass
    def create(self,nx,choice,distri=0,vm=0,entry='datafiles/alfvenwave128_preglass.00000'):
        #--general parameters
         ti=4
  ...
  #          self.npart=len(self.x)
   #         volume           = np.inner(xmaxleft-xminleft,xmaxright-xminright)
    ...
    def adjust_shock_boundaries(self,xleft,xright,dxleft):
            ...
            fac = -12*(int(1.99*2./6.) + 1)*max(dxleft,dxright)
            print("FACTOR",fac)
