...
# test_cases/<test>/ and -r saves relative to there).
#
...
#   run_changa_ci_test.sh         [-a..-h v] <changa-src> <test> <Nsmooth> <Nres> <distrib> <reg-tol>   # gate one
#   run_changa_ci_test.sh --bless [-a..-h v] <changa-src> <test> <Nsmooth> <Nres> <distrib>             # bless one
...
CHANGA_SRC="${1:?usage: run_changa_ci_test.sh [--bless] [-a..-h v] <changa-src> [<test> <Nsmooth> <Nres> <distrib> [reg-tol]]}"
...
# --- BUILD (idempotent: skip once ChaNGa.smp exists) ------------------------
if [ ! -x "$CHANGA_SRC/ChaNGa.smp" ]; then
    mkdir -p "$EXT"
...
do_bless() {
    local t=$1 ns=$2 nr=$3 d=$4; shift 5            # drop reg-tol ($5) too
    ./runtest.sh -r "$@" "$t" "$ns" "$nr" "$d" 1 || { echo "FAIL[bless]: $t"; return 1; }
    echo "BLESSED: test_cases/$t/references/ (commit it)"
}

# GATE: run, then compare against the committed reference.
do_gate() {
    local t=$1 ns=$2 nr=$3 d=$4 tol=$5; shift 5
    [ -n "$tol" ] || { echo "FAIL[no-tol]: $t (reg-tol required for gate mode)"; return 1; }
    ./runtest.sh "$@" "$t" "$ns" "$nr" "$d" 1 || { echo "FAIL[run]: $t"; return 1; }
    local rundir
...
# Run every row of ci/matrix.txt (mode: gate|bless). Rows are
# "<test> <Nsmooth> <Nres> <distrib> <reg-tol> [-a..-h v ...]"; the optional
# trailing flags let one test appear several times (e.g. sedov -a 0/1/2, each a
# distinct <runname>/reference). Runs ALL rows (does not stop on the first
# failure) and exits nonzero iff any failed, mirroring the per-step CI workflow.
run_matrix() {
    local mode=$1 fails=0 ran=0 t ns nr d tol rest
    [ -f ci/matrix.txt ] || { echo "FATAL: ci/matrix.txt not found"; exit 1; }
    while read -r t ns nr d tol rest <&3; do
        { [ -z "$t" ] || [ "${t#\#}" != "$t" ]; } && continue   # skip blanks/comments
        ran=$((ran + 1))
        if [ "$mode" = bless ]; then
            do_bless "$t" "$ns" "$nr" "$d" "$tol" $rest || fails=$((fails + 1))
        else
            do_gate  "$t" "$ns" "$nr" "$d" "$tol" $rest || fails=$((fails + 1))
        fi
    done 3< ci/matrix.txt
    echo "----"
...
if [ -z "$TEST" ]; then
    ...
    if [ "$BLESS" -eq 1 ]; then run_matrix bless; else run_matrix gate; fi
    exit 0
fi

if [ "$BLESS" -eq 1 ]; then
    do_bless "$TEST" "$NSM" "$NRES" "$DIST" "$TOL" "${PARAM_FLAGS[@]+"${PARAM_FLAGS[@]}"}" || exit 1
else
    do_gate "$TEST" "$NSM" "$NRES" "$DIST" "$TOL" "${PARAM_FLAGS[@]+"${PARAM_FLAGS[@]}"}" || exit 1
fi
